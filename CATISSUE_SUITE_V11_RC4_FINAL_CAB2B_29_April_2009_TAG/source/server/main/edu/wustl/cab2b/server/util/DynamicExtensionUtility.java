...
public class DynamicExtensionUtility {
	private static DomainObjectFactory domainObjectFactory = DomainObjectFactory
			.getInstance();

	/**
	 * Persist given entity using dynamic extension APIs. Whether to create
	 * dataspace (table) for this entity or not is decided by
	...
	public static EntityInterface persistEntity(EntityInterface entity) {
		...
		try {
			...
			throw new RuntimeException(
					"Unable to persist Entity in Dynamic Extension", e,
	...
	public static EntityGroupInterface persistEntityGroup(
		...
		try {
			entityGroup = entityGroupManager
					.persistEntityGroupMetadata(entityGroup);
	...
	public static EntityInterface getEntityById(Long id) {
		try {
			...
			throw new RuntimeException(
					"Expected associations are not found in the database", e,
					DE_0003);
		} catch (DynamicExtensionsApplicationException e) {
			throw new RuntimeException(
					"Expected associations are not found in the database", e,
	 ...
	 */
	public static AttributeInterface getAttribute(String entityName,
	...
	public static void setSemanticMetadata(SemanticAnnotatableInterface owner,
		...
		for (int i = 0; i < semanticMetadataArr.length; i++) {
			SemanticPropertyInterface semanticProp = domainObjectFactory
					.createSemanticProperty();
			semanticProp.setSequenceNumber(i);
			semanticProp
					.setConceptCode(semanticMetadataArr[i].getConceptCode());
			semanticProp.setTerm(semanticMetadataArr[i].getConceptName());
			semanticProp.setConceptDefinition(semanticMetadataArr[i]
	 ...
	 * 
	 * @param owner
	 *            Owner of the tagged value
	 * @param key
	 *            Key to be used for tagging
	 * @param value
	 *            Actual value of the tag.
	 */
	public static void addTaggedValue(AbstractMetadataInterface owner,
	...
	/**
	 * This method searches for is there any entity group present with given
	 * name. If present it returns the entity group.If not present then it
	 * creates a entity group with given name. Saves it, and returns the saved
	 * Entity Group.
	 * 
	 * @param name
	...
	public static EntityGroupInterface getEntityGroupByName(String name) {
		...
		try {
			entityGroup = EntityManager.getInstance()
					.getEntityGroupByName(name);
		} catch (DynamicExtensionsSystemException e) {
			throw new RuntimeException(
					"Got System exception from Dynamic Extension while fetching entity group",
	...
	public static EntityGroupInterface createEntityGroup() {
		EntityGroupInterface entityGroup = DomainObjectFactory.getInstance()
				.createEntityGroup();
	...
	public static AttributeInterface getAttributeCopy(
			AttributeInterface source, String name) {
	...
	public static AttributeInterface getAttributeCopy(AttributeInterface source) {
		...
		DataType type = Utility.getDataType(source
				.getAttributeTypeInformation());
		DataElementInterface dataEle = source.getAttributeTypeInformation()
				.getDataElement();
		switch (type) {
		case String:
			...
			if (dataEle instanceof UserDefinedDEInterface) {
				UserDefinedDEInterface userDefinedDE = domainObjectFactory
						.createUserDefinedDE();
				for (PermissibleValueInterface val : ((UserDefinedDEInterface) dataEle)
	 ...
	 *            Semantic Metadata array to set.
	 */
	private static void copySemanticProperties(
			AbstractMetadataInterface copyFrom, AbstractMetadataInterface copyTo) {
		for (SemanticPropertyInterface p : copyFrom
	 ...
	 * @param tarEntity
	 *            target enetiyt of the new association
	 * @return new association
	 */
	public static AssociationInterface createNewOneToManyAsso(
			EntityInterface srcEntity, EntityInterface tarEntity) {
	 ...
	 */
	public static RoleInterface getNewRole(AssociationType associationType,
	...
	public static boolean isEntityGroupMetadata(EntityGroupInterface entityGroup) {
		...
		Collection<TaggedValueInterface> taggedValues = entityGroup
		...
		for (TaggedValueInterface taggedValue : taggedValues) {
			...
			if ((tagKey != null && tagValue != null)
					&& (PathConstants.METADATA_ENTITY_GROUP.compareTo(tagKey) == 0 && PathConstants.METADATA_ENTITY_GROUP
							.compareTo(tagValue) == 0)) {
	...
	private static boolean isSuccessfullyLoaded(
			Collection<TaggedValueInterface> taggedValues) {
		for (TaggedValueInterface taggedValue : taggedValues) {
			String taggedKey = taggedValue.getKey();
			String tagggedValue = taggedValue.getValue();
			if ((taggedKey != null && tagggedValue != null)
					&& (ServerConstants.LOAD_STATUS.compareTo(taggedKey) == 0 && ServerConstants.LOAD_FAILED
							.compareTo(tagggedValue) == 0))
				return false;
		}
		return true;
	}
