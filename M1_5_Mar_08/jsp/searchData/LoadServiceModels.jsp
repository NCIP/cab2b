...
<%
 List<EntityGroupInterface> loadEntity = (List) session.getAttribute(Cab2bConstants.FILTERED_LOADED_MODELS); 
                    ...
                    <tr>
                      <td align="left"><table border="0" cellpadding="2" cellspacing="0">
                        ...
                        <tr>
                          ...
                          <td align="left"> <span class="font_blk_s"> Search For </span></td></td>
			...
			for (int i=offset.intValue(),l = Math.min(i + maxPageItems, loadEntity.size());	i < l; i++)
				{
					...
					String pagNumber = request.getParameter(Cab2bConstants.OFFSET_PARAMETER);
		                                                
%>         
		
		  <pg:item >                      
                      <tr>
                        <td align="left" colspan="2"><span class="font_bl1_b" onmouseover="Tip('<%=descriptionWithoutComma%>',400,200)"><a href="SearchServiceInstances.action?serviceName=<%=loadEntity.get(i).getLongName()%>&fromPage=<%=pagNumber%>&searchString=${param.textbox}" class="set4"><%=loadEntity.get(i).getLongName()%> </a></span><br />
...
<%} %>                     
 ...
 { %>
    		...
    		<tr>
                        <td align="left" ><span class="font_bl1_b"><a href="LoadServiceModels.action" class="set4">Show All Models</a></span><br />
