...
public class CaDSRLoadModel extends BaseAction implements ServletRequestAware, SessionAware {
    ...
    public String execute() {
        Map<String, List<String>> allSelectedModelList = (Map<String, List<String>>) this.session.get(Cab2bConstants.ALL_SELECTED_MODELS_LIST);
        if (allSelectedModelList == null) {
            allSelectedModelList = new HashMap<String, List<String>>();
        }

        final String offset = this.servletRequest.getParameter(Cab2bConstants.OFFSET_PARAMETER);
        if (offset != null) {//This block runs when user selected some model and goes to next page
            if (getCheckboxValues() != null) {
                ...
                this.session.put(Cab2bConstants.ALL_SELECTED_MODELS_LIST, allSelectedModelList);
        ...
        if (this.textbox != null && !this.textbox.equals("")) {//This if code for search model
            this.allModelList = (List<CaDSRModelDetailsBean>) this.session.get(Cab2bConstants.ALL_AVAILABLE_MODELS_TO_LOAD);
            this.session.put(Cab2bConstants.FILTERED_AVAILABLE_MODELS_TO_LOAD, getSearchedModels(this.allModelList));
        } else {
            if (this.session.get(Cab2bConstants.ALL_AVAILABLE_MODELS_TO_LOAD) != null) {
                this.session.put(Cab2bConstants.FILTERED_AVAILABLE_MODELS_TO_LOAD, this.session.get(Cab2bConstants.ALL_AVAILABLE_MODELS_TO_LOAD));
            } else {
                try {
                ...
                this.session.put(Cab2bConstants.ALL_AVAILABLE_MODELS_TO_LOAD, this.allModelList);
                this.session.put(Cab2bConstants.FILTERED_AVAILABLE_MODELS_TO_LOAD, this.allModelList);
            }
        }
