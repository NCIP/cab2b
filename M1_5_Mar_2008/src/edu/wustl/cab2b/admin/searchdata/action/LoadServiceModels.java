...
public class LoadServiceModels extends BaseAction implements ServletRequestAware, SessionAware {
    ...
    public String execute() {
        ...
        String offset = (String) servletRequest.getParameter(Cab2bConstants.OFFSET_PARAMETER);
        String callFrom = (String) servletRequest.getParameter("callFrom");
        if (callFrom != null) {
            session.remove(Cab2bConstants.ALL_LOADED_MODELS);
            session.remove(Cab2bConstants.FILTERED_LOADED_MODELS);
        }
        if (offset != null) {
            return Cab2bConstants.SUCCESS;
        }
        if (textbox != null && !textbox.equals("")) {
            allEntityList = (List<EntityGroupInterface>) session.get(Cab2bConstants.ALL_LOADED_MODELS);
            getSearchedModels(allEntityList, searchedEntitylList);
            session.put(Cab2bConstants.FILTERED_LOADED_MODELS, searchedEntitylList);
        } else {
            if (session.get(Cab2bConstants.ALL_LOADED_MODELS) != null)
                session.put(Cab2bConstants.FILTERED_LOADED_MODELS,
                            (List<EntityGroupInterface>) session.get(Cab2bConstants.ALL_LOADED_MODELS));
            else {
                allEntityList.addAll((Collection<EntityGroupInterface>) new ServiceInstanceBizLogic().getMetadataEntityGroups());
                session.put(Cab2bConstants.ALL_LOADED_MODELS, allEntityList);
                session.put(Cab2bConstants.FILTERED_LOADED_MODELS, allEntityList);
            }
        }
        return Cab2bConstants.SUCCESS;
    }
