 ...
 * @author chetan_patil
 */
public class SearchCategory extends BaseAction implements ServletRequestAware, ServletResponseAware {
    ...
    public String getPermissibleValuesChecked() {
        //   setPermissibleValuesChecked("off");
    ...
    public String execute() throws IOException, NumberFormatException, CheckedException {
        //TODO Set a common logger for the project
        Logger.configure();
        ...
        if (textfieldValue != null && !textfieldValue.equals("")) {
            ...
            for (EntityInterface entity : resultList) {
                if (!Utility.isCategory(entity)) {
                    ...
                    entitiesString.append(";" + entityName + "|" + entityId + "|" + description);
                }
            }
    ...
    private int[] prepareSearchTarget() {
        ...
        if (classCheckBoxChecked != null
                && (classCheckBoxChecked.equalsIgnoreCase("on") || classCheckBoxChecked.equalsIgnoreCase("true"))) {
            if (includeDescriptionChecked != null
                    && (includeDescriptionChecked.equalsIgnoreCase("on") || includeDescriptionChecked.equalsIgnoreCase("true"))) {
                target.add(new Integer(edu.wustl.cab2b.common.util.Constants.CLASS_WITH_DESCRIPTION));
            } else {
                target.add(new Integer(edu.wustl.cab2b.common.util.Constants.CLASS));
            }
        }
        if (attributeCheckBoxChecked != null
                && (attributeCheckBoxChecked.equalsIgnoreCase("on") || attributeCheckBoxChecked.equalsIgnoreCase("true"))) {
            if (includeDescriptionChecked != null
                    && (includeDescriptionChecked.equalsIgnoreCase("on") || includeDescriptionChecked.equalsIgnoreCase("true"))) {
                target.add(new Integer(edu.wustl.cab2b.common.util.Constants.ATTRIBUTE_WITH_DESCRIPTION));
            } else {
                target.add(new Integer(edu.wustl.cab2b.common.util.Constants.ATTRIBUTE));
            }
        }
        if (permissiblevaluesCheckBoxChecked != null
                && (permissiblevaluesCheckBoxChecked.equalsIgnoreCase("on") || permissiblevaluesCheckBoxChecked.equalsIgnoreCase("true"))) {
