...
public class ServiceInstances extends BaseAction implements RequestAware, SessionAware {
    ...
    public String execute() {
        ...
        String offset = (String) request.get(Cab2bConstants.OFFSET_PARAMETER);
        String callFrom = (String) request.get("callFrom");
        if(callFrom!=null)
        {
            session.remove(Cab2bConstants.ALL_SERVICE_INSTANCES);
            session.remove(Cab2bConstants.FILTERED_SERVICE_INSTANCES);
        }
        if (offset != null ) {
            return Cab2bConstants.SUCCESS;
        }
        if (textbox != null && !textbox.equals("")) {
            allServiceInstances = (List<AdminServiceMetadata>) session.get(Cab2bConstants.ALL_SERVICE_INSTANCES);
            filteredServiceInstances.clear();
            getSearchedModels(allServiceInstances, filteredServiceInstances);
            session.put(Cab2bConstants.FILTERED_SERVICE_INSTANCES, filteredServiceInstances);
        } else {
            if (session.get(Cab2bConstants.ALL_SERVICE_INSTANCES) != null)
                session.put(Cab2bConstants.FILTERED_SERVICE_INSTANCES,
                            (List<AdminServiceMetadata>) session.get(Cab2bConstants.ALL_SERVICE_INSTANCES));
            else {
                ...
                try {
                    ...
                    session.put(Cab2bConstants.ALL_SERVICE_INSTANCES, allServiceInstances);
                    session.put(Cab2bConstants.FILTERED_SERVICE_INSTANCES, allServiceInstances);
                } catch (Exception e) {
