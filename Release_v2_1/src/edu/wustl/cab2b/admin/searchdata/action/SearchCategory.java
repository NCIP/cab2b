...
public class SearchCategory extends BaseAction {
	...
	private int[] prepareSearchTarget() {
		...
		boolean includeDesc = isChecked(includeDescriptionChecked);
		List<Integer> target = new ArrayList<Integer>();

		if (isChecked(classCheckBoxChecked)) {
			if (includeDesc) {
		...
		if (isChecked(attributeCheckBoxChecked)) {
			if (includeDesc) {
		...
		if (isChecked(permissiblevaluesCheckBoxChecked)) {
			target.add(PV);
		}
		int[] searchTarget = new int[target.size()];
		for (int i = 0; i < target.size(); i++) {
			searchTarget[i] = target.get(i);
		}
		return searchTarget;
	}
