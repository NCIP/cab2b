...
public class DefaultSpreadSheetViewPanel extends Cab2bPanel implements
	...
	protected ImageIcon createImageIcon(String pathImage) {
		...
		if (imgURL != null) {
			...
			System.err
					.println("[DefaultSpreadSheetViewPanel]: Unable to locate Toolbar icon images: "
							+ pathImage);
			return null;
		}
	}
	...
	private void initializeGUI() {
		...
		if (expGridPanel != null) {
			Action saveCategoryAction = new SaveCategoryActionListener(
					expGridPanel);
			...
			spreadsheet
					.removeComponentFromToolBar(JSheet.MENU_BUTTON_PROPERTIES);
			...
			spreadsheet
					.removeComponentFromToolBar(JSheet.MENU_BUTTON_ADD_COLUMN);
		...
		spreadsheet
				.addAncestorListener(new javax.swing.event.AncestorListener() {
					...
					public void ancestorAdded(
							javax.swing.event.AncestorEvent evt) {
						firePropertyChange(SPREADSHEET_MODEL_INSTALLED, null,
								spreadsheet);
					}

					public void ancestorRemoved(
							javax.swing.event.AncestorEvent evt) {
						firePropertyChange(SPREADSHEET_MODEL_UNINSTALLED, null,
								spreadsheet);
					}
	...
	public List<IRecord> getAllVisibleRecords() {
		...
		if (isExtraColumnAdded()) {
			...
			for (int rowIndex = 0; rowIndex < rowCount
					&& rowIndex < records.size(); rowIndex++) {
				int modelRowNumber = tableModel
				...
				for (AttributeInterface attribute : record.getAttributes()) {
					...
					newAttribute.setAttributeTypeInformation(attribute
							.getAttributeTypeInformation());
					mapAttributeValue.put(newAttribute, record
							.getValueForAttribute(attribute));
				}

				// Now add user defined attributes
				for (int columnIndex = 0; columnIndex < tableModel
						.getColumnCount(); columnIndex++) {

					int modelColIndex = tableModel
							.convertColumnIndexToModel(columnIndex);
					TableModel tblModel = spreadsheet.getJSheetTableModel();
					if (isUserColumnDefined(modelColIndex)) {
						AttributeInterface newAttribute = DomainObjectFactory
								.getInstance().createStringAttribute();
						newAttribute.setName(tblModel
								.getColumnName(modelColIndex));
						mapAttributeValue.put(newAttribute, tblModel
								.getValueAt(modelRowNumber, modelColIndex));
					}
				}

				// Fill data in the records
				IRecord newRecord = QueryResultFactory.createRecord(
						mapAttributeValue.keySet(), new RecordId("", ""));
				Set<AttributeInterface> newAttributeSet = mapAttributeValue
						.keySet();
				for (AttributeInterface newAttribute : newAttributeSet) {
					newRecord.putValueForAttribute(newAttribute,
							mapAttributeValue.get(newAttribute));
				}
	...
	private boolean isUserColumnDefined(int viewColumnIndex) {
		return spreadsheet.getJSheetTableModel().isCellEditable(0,
				viewColumnIndex);
	}
	...
	public List<IRecord> getSelectedRecords() {
		...
		for (int i = 0; i < selectedRowCount && i < records.size(); i++) {
			int recordNumber = spreadsheet.getViewTableModel()
					.convertRowIndexToModel(spreadsheet.getSelectedRows()[i]);
			selectedRecords.add(records.get(recordNumber));
		}
		return selectedRecords;
	}
	...
	class ShowRecordDetailsListener implements PropertyChangeListener {
		public void propertyChange(PropertyChangeEvent evt) {
			if (evt.getPropertyName().equals(
					spreadsheet.REQUESTED_SHOW_ROW_DETAILS)) {
				Integer rowNumber = (Integer) evt.getNewValue();
				DefaultDetailedPanel defaultDetailedPanel = ResultPanelFactory
						.getResultDetailedPanel(records.get(rowNumber
								.intValue()));
				defaultDetailedPanel
						.addPropertyChangeListener(new PropertyChangeListener() {
							public void propertyChange(PropertyChangeEvent evt) {
								firePropertyChange(evt.getPropertyName(), evt
										.getOldValue(), evt.getNewValue());
							}
						});
				if (expGridPanel != null) {
					expGridPanel.addDetailTabPanel("Details_"
							+ (rowNumber.intValue() + 1), defaultDetailedPanel);
					firePropertyChange(DISABLE_CHART_LINK, evt.getOldValue(),
							evt.getNewValue());
				} else {
	...
	public List<String> getSelectedColumnNames() {
		List<String> selectedColumnNames = new ArrayList<String>(
				getSelectedColumns().length);
		for (int i = 0; i < getSelectedColumns().length; i++) {
			selectedColumnNames.add(getSpreadSheetDataModel().getColumnName(
	...
	public Object[][] getSelectedData() {
		...
		for (int rowIndex = 0; rowIndex < getSelectedRows().length; rowIndex++)
			for (int columnIndex = 0; columnIndex < getSelectedColumns().length; columnIndex++) {
				data[rowIndex][columnIndex] = getSpreadSheetDataModel()
						.getValueAt(getSelectedRows()[rowIndex],
	...
	public List<AttributeInterface> getUserDefinedAttributes() {
		...
		for (int i = 0; i < spreadsheet.getJSheetTableModel().getColumnCount(); i++) {
			if (isUserColumnDefined(i)) {
				AttributeInterface newAttribute = DomainObjectFactory
						.getInstance().createStringAttribute();
				newAttribute.setName(spreadsheet.getJSheetTableModel()
						.getColumnName(i));
