...
public class ServiceInstanceBizLogic {
     ...
     */
    public List<AdminServiceMetadata> getServiceMetadataObjects(String serviceName) throws MalformedURIException, RemoteException {
     ...
     * @throws RemoteException 
     */
    private List<AdminServiceMetadata> getMetadataObjects(final EndpointReferenceType[] services, String serviceName) throws RemoteException {
        ...
        for (EndpointReferenceType service : services) {
            ...
            try {
                ...
                Logger.out.error(e.getMessage(), e);
                metadataObject.setSeviceDescription("");
                metadataObject.setHostingResearchCenter("");
            }
    ...
    public Collection<EntityGroupInterface> getMetadataEntityGroups() {
        ...
        for (EntityGroupInterface entityGroup : entityGroups) {
            if (Constants.CATEGORY_ENTITY_GROUP_NAME.equalsIgnoreCase(entityGroup.getLongName()) || Constants.DATALIST_ENTITY_GROUP_NAME.equalsIgnoreCase(entityGroup.getLongName())) {
                continue;
            }
    ...
    public void saveServiceInstances(Collection<AdminServiceMetadata> serviceMetadataObjects, User user) throws RemoteException {

        for (AdminServiceMetadata serviceMetadata : serviceMetadataObjects) {
            String entityGroupName = serviceMetadata.getSeviceName();
            try {
                EntityGroupInterface groupInterface = EntityManager.getInstance().getEntityGroupByName(entityGroupName);
    ...
    //        User user = null;
    //        if (userName != null) {
    //            //TODO Correct this after usercase changes
    //            //currently only admin is supposed to load service instances, thus admin user is returned.
