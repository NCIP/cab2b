...
public class SearchNavigationPanel extends Cab2bPanel implements ActionListener {
	...
	private void initGUI() {
		...
		saveConditionButton
				.addActionListener(new SaveConditionButtonActionListener());
	...
	public void showCard(boolean blnNext) {
		...
		if (blnNext && ((iSelectedCard + 1) < centerPanel.getIdentifierCount())) {
			 ...
			 */
			layout.show(centerPanel, centerPanel
					.getIdentifier(iSelectedCard + 1));
	...
	private void setButtons() {
		int cardindex = m_mainSearchPanel.getCenterPanel()
	...
	public void gotoDataListPanel(final IDataRow datarow) {
		SearchCenterPanel searchCenterPanel = m_mainSearchPanel
				.getCenterPanel();
		DataListPanel dataListPanel = (DataListPanel) searchCenterPanel.m_arrCards[searchCenterPanel
		...
		searchCenterPanel
				.add(dataListPanel, SearchCenterPanel.m_strDataListlbl);
		showCard(true);
	}
	...
	private class NextButtonActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			final SearchCenterPanel searchCenterPanel = m_mainSearchPanel
					.getCenterPanel();
			final IClientQueryBuilderInterface clientQueryBuilder = m_mainSearchPanel
					.getQueryObject();
			SearchNavigationPanel.messageLabel.setText("");

			if (searchCenterPanel.getSelectedCardIndex() == 0) {
				AddLimitPanel addLimitPanel = searchCenterPanel
						.getAddLimitPanel();
				addLimitPanel.addSearchPanel(searchCenterPanel
						.getChooseCategoryPanel().getSearchPanel());
				...
				CustomSwingWorker swingWorker = new CustomSwingWorker(
						SearchNavigationPanel.this) {

					@Override
					protected void doNonUILogic() throws Exception {
						if (clientQueryBuilder != null
								&& clientQueryBuilder.getVisibleExressionIds()
										.size() > 0) {
							if (AdvancedDefineViewPanel.isMultipleGraphException == true) {
								...
								if (null != searchCenterPanel.m_arrCards[2]) {
									searchCenterPanel
											.remove(searchCenterPanel.m_arrCards[2]);
								}
					...
					protected void doUIUpdateLogic() throws Exception {
						if (clientQueryBuilder == null
							...
							// rule.
							JOptionPane.showMessageDialog(m_mainSearchPanel
									.getParent(),
					...
					protected void doUIUpdateLogic() {
						if (queryResults != null) {
							...
							if (recordNo == 0) {
								...
								if (SearchNavigationPanel.this.m_mainSearchPanel
										.isParaQueryShowResultButtonPressed()) {
									// close the search wizard
									NewWelcomePanel.mainFrame
											.closeSearchWizardDialog();
									return;
								} else {
									gotoAddLimitPanel();
								}
								...
								if (searchCenterPanel.m_arrCards[3] != null) {
									searchCenterPanel
											.remove(searchCenterPanel.m_arrCards[3]);
								}
	...
	private class PreviousButtonActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			if (cardIndex == 1) {
				...
				ChooseCategoryPanel chooseCategoryPanel = searchCenterPanel
						.getChooseCategoryPanel();
				chooseCategoryPanel.addSearchPanel(searchCenterPanel
						.getAddLimitPanel().getSearchPanel());
				searchCenterPanel.setChooseCategoryPanel(chooseCategoryPanel);
			} else if (cardIndex == 2) {
				AddLimitPanel addLimitPanel = (AddLimitPanel) m_mainSearchPanel
						.getCenterPanel().getAddLimitPanel();
				MainDagPanel mainDagPanel = addLimitPanel.getMainDagPanel();

				if (mainDagPanel.getVisibleNodeCount() == 0
						&& mainDagPanel.getExpressionCount() > 0) {

					mainDagPanel.updateGraph();
					addLimitPanel.editAddLimitUI(mainDagPanel
							.getFirstExpression());
					mainDagPanel.selectNode((ExpressionId) mainDagPanel
							.getFirstExpression().getExpressionId());
				}
			}
			showCard(false);
		}
