...
public class QueryConverterTest extends TestCase {
    ...
    public void testIsORedQuery() {
        ...
        assertEquals(Boolean.FALSE, new QueryConverter().isORedQuery(query));
    }
    ...
    public void testChangeConditionOperatorPositive() {
        ...
        ICab2bQuery oredQuery = queryConverter.changeConditionOperator(query);

        assertEquals(Boolean.TRUE, queryConverter.isORedQuery(oredQuery));
    }

    public void testExecuteORedQuery() {
        ICab2bQuery oredQuery = new MockQueryObjects().createQuery_Gene_mRNA_Protein();
        assertEquals(Boolean.TRUE, new QueryConverter().isORedQuery(oredQuery));
