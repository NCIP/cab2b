				...
				<TD>
					<DIV>
						...
						<logic:notEqual name="queryCount" value="1">
							<SELECT class="select" name="savedQueries" onChange="document.getElementById('centerpanelcontent').innerHTML='';processAJAXRequest('KeywordSearch.do?savedQueries=' + this.value, 'centerpanelcontent');updateView()"/>
 ...
 function updateView()
{
   ...
   for(var i=0;i<spanObjs.length;i++)
  {
     if(spanObjs[i].className=='pagebanner')
    {
	  ...
	  pagebanner = "<span class='pagebanner'>" + spanObjs[i].innerHTML + "</span>";
    }
     if(spanObjs[i].className=='pagelinks')
    {
	  spanObjs[i].style.display = 'none';
	  pagelinks = "<span class='pagelinks'>" + spanObjs[i].innerHTML + "</span>";
    }
  }
  ...
  document.getElementById('centerpanelcontent').innerHTML = "<table id='rec' cellpadding='4' class='simple' cellspacing='1'>" + document.getElementById('rec').innerHTML + "</table>";
  document.getElementById("top").innerHTML = pagebanner + pagelinks;
