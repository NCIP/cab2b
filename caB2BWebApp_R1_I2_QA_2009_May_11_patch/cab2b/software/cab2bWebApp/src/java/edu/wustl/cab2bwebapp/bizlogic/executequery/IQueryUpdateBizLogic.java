 ...
 *
 */
public class IQueryUpdateBizLogic {
    private static final Logger logger = edu.wustl.common.util.logger.Logger.getLogger(IQueryUpdateBizLogic.class);
        ...
        for (IExpression expression : constraints) {
            ...
            for (int i = 0; i < no_of_oprds; i++) {
                ...
                if (operand instanceof IRule) {
                    ...
                    errorMessage = componentValues(displayNamesMap, errorMessage, newConditions, expId, rule,
                                                   query);
                    if (displayNamesMap == null && (Collections.list((IRule) operand)).size() == 0) {
                        IExpression expression1 = rule.getContainingExpression();
                        AttributeInterface attributeObj = getIdNotNullAttribute(expression1.getQueryEntity().getDynamicExtensionsEntity());
    ...
    public Map<String, String[]> createConditionsMap(String queryString) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!condition.equals("")) {
                condition = condition.substring(AddLimitConstants.ARGUMENT_ZERO,
                                                condition.indexOf(AddLimitConstants.ENTITY_SEPARATOR));
                String attrName = null;
                StringTokenizer tokenizer = new StringTokenizer(condition,
                        AddLimitConstants.QUERY_OPERATOR_DELIMITER);
        ...
        for (int j = 0; j < size; j++) {
            ...
            if (newConditions != null && newConditions.containsKey(componentName)) {
                ...
                if (displayNamesMap != null && !(displayNamesMap.containsKey(componentName))) {
                    ...
                    condition.setRelationalOperator(RelationalOperator.getOperatorForStringRepresentation(params[AddLimitConstants.INDEX_PARAM_ZERO]));
                }
            }
    ...
    private String validateAttributeValues(String dataType, List<String> attrvalues) {
        ...
        for (String enteredValue : attrvalues) {
            ...
            } else if ((AddLimitConstants.BIG_INT.equalsIgnoreCase(dataType) || AddLimitConstants.INTEGER.equalsIgnoreCase(dataType))
    ...
    private String getErrorMessageForPositiveNum(String errorMessages, String enteredValue) {
        errorMessages = errorMessages + "<li><font color\\='red'>"
                + ApplicationProperties.getValue("simpleQuery.intvalue.poisitive.required") + "</font></li>";
