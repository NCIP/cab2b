...
public class AddLimitHTMLGeneratorBizLogic {
    ...
    public AddLimitHTMLGeneratorBizLogic(String parseFilePath) {
        try {
            ...
            StringBuffer error = new StringBuffer(
                    "Error while generating the metadata for HTML generator! Please check file.");
    ...
    private String createTableForSavedQuery() {
        ...
        generatedHTML.append("<tr><td class='labelpanel' valign='top'>"
    ...
    private StringBuffer generateSaveQueryForEntity(Integer expressionID, EntityConditions entityConditions,
        ...
        Map<String, ICondition> attribNameConditionMap = getAttributeNameConditionMap(entityConditions.getConditions());
        ...
        for (AttributeInterface attribute : entityAttributes) {
            ...
            if (attribNameConditionMap.containsKey(attributeName)) {
                ...
                if (isBGColor = !isBGColor) {
                    styleSheetClass = "formfieldspanelalternate";
                }
                generatedHTML.append("\n<tr class='" + styleSheetClass + "'" + " id='componentId'>\n");

                IParameter<?> paramater = getParameterForCondition(attribNameConditionMap.get(attributeName),
                                                                   parameterList);
                if (paramater != null) {
                    attributesList += generateComponentName(expressionID, attribute) + ";";

                    generatedHTML.append("<td valign='top' align='left' class='querylimittext' nowrap='nowrap' width=\"15%\">"
                            + paramater.getName());
                ...
                generatedHTML.append(generateHTMLForConditions(attribute, attribNameConditionMap, parameterList)).append(
                                                                                                                         "\n</tr>");
            }
        }
