...
public class IndexServiceOperations {
    ...
    static ServiceURL merge(ServiceURL urlsFromDatabase, ServiceURL urlsFromIndexService) {
        ...
        if (hostingCenterName.length() != 0 && hostingCenterName.length() > 0) {
        ...
        if (urlsFromDatabase.getDomainModel() == null || urlsFromDatabase.getDomainModel().length() == 0) {
            urlsFromDatabase.setDomainModel(domainModel);
        }
        String version = urlsFromIndexService.getVersion();
        if (urlsFromDatabase.getVersion() == null || urlsFromDatabase.getVersion().length() == 0) {
