...
public class ExecuteQueryAction extends Action {
    ...
    /**
     * The execute method is called by the struts action flow and it calls the code for retrieve the query results.
     * @param mapping
     * @param form
     * @param request
     * @param response
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        Collection<ICab2bQuery> queriesForScriplet = searchResults.keySet();
        String selectedUrl = urlsForSelectedQueries.get(0);
        List<Map<AttributeInterface, Object>> finalResult = null;
        for (ICab2bQuery queryObjScr : queriesForScriplet) {
            if (queryObjScr.getName().equals(selectedQueryName)) {
                TransformedResultObjectWithContactInfo resultTrans = searchResults.get(queryObjScr);
                finalResult = resultTrans.getResultForUrl(selectedUrl);
                break;
            }
        }
