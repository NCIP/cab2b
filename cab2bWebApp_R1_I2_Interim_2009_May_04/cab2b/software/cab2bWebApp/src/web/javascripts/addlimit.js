...
//Function to auto select first query in the saved query panel and show the add limit form corresponding to that query.
 function showFirstQueryForm()
{
  var objs = document.getElementsByName("savedquery");
   if(objs[0])
  {
    objs[0].className = 'selectedquery';
	document.getElementById('queryname').innerHTML = objs[0].innerHTML;
	processAJAXRequest('AddLimit.do?queryId=' + objs[0].id, 'definelimitspanelbody');
	document.getElementsByName("queryId")[0].value = objs[0].id;
	clearTimeout("showFirstQueryForm()");
  }
   else
  {
	setTimeout("showFirstQueryForm()", 1);
  }
}

//Function to update links display style on selection of saved queries. Selected saved query link should look different from other saved query links.
 function updateLinks(obj)
{
   ...
   for(var i=0;i<objs.length;i++)
  {
	 if(objs[i].name==obj.name)
	{
	  ...
	  document.getElementsByName("queryId")[0].value = objs[i].id;
	}
	 else
	{
...
//Function to make proper changes in the add limit form on onchange event in select boxes on the form.
...
//Function to make proper changes in the add limit form on onclick event in option buttons on the form.
var radio = "";
var toggleRadio = "";
 function resetOptionButton(id, currentObj)
{ 
  //Variable radio keeps track of which radio button is selected (true/false).
...
//Function to create string object representing the changes done on the Add Limit form. This string object will be passed to the action class query object will be modified accordingly.
 function createQueryString()
{   
   if(document.getElementById('attributesList').value != null)
  {		  
	var queryString =  createStandardQueryString(document.getElementById('attributesList').value);     
	document.getElementById('conditionList').value = queryString;
     if(document.getElementById("totalCF").value != 0)
	{
	  var buildTQStr = createQueryStringForExcecuteSavedTQ(totalCFCount);
  ...
  { 
	document.getElementById("definelimitspanelbody").innerHTML += "<INPUT type='hidden' name='conditionList' value='" + queryString + "'>"
  }		
