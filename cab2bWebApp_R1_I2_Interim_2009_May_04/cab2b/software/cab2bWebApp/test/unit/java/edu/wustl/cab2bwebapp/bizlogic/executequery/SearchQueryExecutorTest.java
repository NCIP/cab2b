...
public class SearchQueryExecutorTest extends TestCase {
    ...
    public void testInsetKeywordInQueries() {
        ICab2bQuery query = new MockQueryObjects().createCaFEGeneQuery();
        ICab2bQuery keywordQuery = new QueryConverter().changeConditionOperator(query);

        final String keyword = "gmb";
        new SearchQueryExecutor().insertKeywordInQueries(keywordQuery, keyword);
