...
public class CuratePathBizLogic {
	 ...
	 * @return
	 */
	public boolean shallSaveCuratedPath(CategoryDagPanel categoryDAGPanel,
			Map<String, IPath> idVsPathMap, boolean isSelected) {
		CuratedPath curatedPath = createCuratedPath(categoryDAGPanel,
				idVsPathMap, isSelected);

		return shallSaveCuratedPath(curatedPath);
	}

	private boolean shallSaveCuratedPath(final CuratedPath curatedPath) {
	 ...
	 * @return
	 */
	private CuratedPath createCuratedPath(CategoryDagPanel categoryDAGPanel,
			Map<String, IPath> idVsPathMap, boolean isSelected) {
	 ...
	 * @param entitySet
	 */
	private void addEntityInterface(Set<DAGNode> dagNodeSet, DAGLink dagLink,
			Set<EntityInterface> entitySet) {
		for (DAGNode dagNode : dagNodeSet) {
			if (dagLink.getSourceNodeId() == dagNode.getNodeId()) {
				entitySet.add(EntityCache.getInstance().getEntityById(
						dagNode.getEntityId()));
			}
