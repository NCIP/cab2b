...
public class CaDSRLoadModel extends BaseAction {
	...
	public String execute() {
		Map<String, List<String>> allSelectedModelList = (Map<String, List<String>>) this.session
		...
		if (offset != null) {// This block runs when user selected some model
			...
			if (getCheckboxValues() != null) {
				final List<String> selectedModels = new ArrayList<String>(
						getCheckboxValues().length);
				...
				this.session
						.put(ALL_SELECTED_MODELS_LIST, allSelectedModelList);
		...
		if (this.textbox != null && !this.textbox.equals("")) { // This code is
			// for search
			// model
			this.allModelList = (List<CaDSRModelDetailsBean>) this.session
					.get(ALL_AVAILABLE_MODELS_TO_LOAD);
			this.session.put(FILTERED_AVAILABLE_MODELS_TO_LOAD,
					getSearchedModels(this.allModelList));
		} else {
			if (this.session.get(ALL_AVAILABLE_MODELS_TO_LOAD) != null) {
				this.session.put(FILTERED_AVAILABLE_MODELS_TO_LOAD,
						this.session.get(ALL_AVAILABLE_MODELS_TO_LOAD));
			} else {
				if (toFetch()) {
					try {
						synchronized (this) {
							boolean isListModelNull = servletContext
									.getAttribute(AdminConstants.LIST_OF_MODELS) == null ? true
									: false;
							if (isListModelNull) {
								this.allModelList = new CaDSRModelBizLogic()
										.getProjectsDisplayDetails();
								servletContext.setAttribute(
										AdminConstants.LIST_OF_MODELS,
										this.allModelList);
							}
						}
					...
					this.allModelList = (List<CaDSRModelDetailsBean>) servletContext
							.getAttribute(AdminConstants.LIST_OF_MODELS);
				}
				this.session.put(ALL_AVAILABLE_MODELS_TO_LOAD,
						this.allModelList);
				this.session.put(FILTERED_AVAILABLE_MODELS_TO_LOAD,
						this.allModelList);
			}
	 ...
	 * @return
	 */
	public List<CaDSRModelDetailsBean> getSearchedModels(
			List<CaDSRModelDetailsBean> allModels) {
	...
	private boolean toFetch() {
		...
		Long prevTime = (Long) servletContext
				.getAttribute(AdminConstants.TIME_OF_MODEL_FETCHED);
		long refreshTime = Long.parseLong(ServerProperties
				.getCadsrRefreshTime());
		boolean isListModelNull = servletContext
				.getAttribute(AdminConstants.LIST_OF_MODELS) == null ? true
				: false;
		if (prevTime == null || (currentTime - prevTime) >= refreshTime
				|| isListModelNull) {
			servletContext.setAttribute(AdminConstants.TIME_OF_MODEL_FETCHED,
	...
	public boolean searchName(String modelName, String searchName) {
		...
		while (tokenizer.hasMoreTokens()) {
			if (tokenizer.nextToken().toLowerCase().contains(
