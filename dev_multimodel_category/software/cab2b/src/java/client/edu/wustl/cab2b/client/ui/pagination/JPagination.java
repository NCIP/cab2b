...
/**
 * A Swing component to perform pagination of long list of user data. This
 * component is designed to simulate the Browser kind of pagination of search
 * results.
 * 
 ...
 * 
 * List of Property Change Listeners implemented and working. 1) isSelectable 2)
 * isGroupActionEnabled // TODO this should automatically get disabled if
 * isSelectable is disabled. 3) elementsPerPage
 * 
 * @author chetan_bh
 */
public class JPagination extends Cab2bPanel implements PropertyChangeListener,
		MouseWheelListener {
	...
	/**
	 * A Panel to display some common action elements like "Select All",
	 * "Clear All", etc.
	 */
	private JGroupActionPanel groupActionPanel;

	/**
	 * A Panel to display the page indexes, for user to navigate through the
	...
	/**
	 * S actionListenerList, so that many listener can register to listen for
	 ...
	 */
	public JPagination(Vector<PageElement> data, Pager pager,
			Component parentComponent, boolean autoPageResize) {
	...
	private void preInitializeGUI(Vector<PageElement> data, Pager pager,
			Component parentComponent, boolean autoPageResize) {
		...
		if (parentComponent == null) {
			...
			parentComponent
					.addComponentListener(new PaginationComponentListener());
		}
	}

	private void initGUI() {
		this.removeAll();
		pageBar = new JPageBar(paginationModel.pager.getAllPageIndices(),
				new Vector(), this);
		...
		/*
		 * if (automaticPageResize) pagePanel.addComponentListener(new
		 * PaginationComponentListener());
		 */
	...
	/**
	 * This method is used to reset all the hyperlinks in JPagination into
	 * labels
	 * 
	 */
	...
	private void addNewMouseWheelListener() {
		MouseWheelListener[] mouseWheelListeners = this
	...
	/*
	 * TODO: this function should now take into account
	 * pageLayoutOrientation and pageDimension attributes into account
	 * while laying out page elements in the page panel.
	 */
	
	/**
	 * Returns a Panel which is embedded with page element components
	 * representing the a page.
	 * 
	 * @param pageElements
	 *            a vector of page elements also called as page.
	 * @return a page panel.
	 * 
	 */
	public JXPanel getPagePanel(Vector<PageElement> pageElements) {
		...
		for (PageElement element : pageElements) {
			...
			PageElementIndex pageElementIndex = new PageElementIndex(
					currentPageIndex, indexInPage);

			JPageElement pageElementPanel = new JPageElement(this, element,
					isSelectable, pageElementIndex);
			pageElementPanel
					.addHyperlinkActionListeners(this.pageElementActionListenerList);
	...
	public void addPageElementActionListener(ActionListener actionListener) {
		for (int i = 0; i < pagePanel.getComponentCount(); i++) {
			JPageElement pageElementComp = (JPageElement) pagePanel
					.getComponent(i);
	...
	// This should set actionlisteners of each pageelements actionlistener to
	// null
	...
	public void setGroupActionEnabled(boolean newValue) {
		if (newValue != this.isGroupActionEnabled) {
			...
			firePropertyChange("isGroupActionEnabled", oldValue, new Boolean(
					newValue));
		}
	}
	...
	public void setElementsPerPage(int newValue) {
		...
		if (newValue != oldValue && newValue > 0) {
			firePropertyChange("elementsPerPage", new Integer(oldValue),
	...
	/**
	 * This function propagates the selectionEvent to selectionModel, where all
	 * the selections are maintained by selectionModels datastructure.
	 * 
	 * @param pageSelectionEvent
	 */
	protected void fireSelectionValueChanged(
			PageSelectionEvent pageSelectionEvent) {
	...
	public Vector getSelectedPageElementsUserObjects() {
		...
		Vector<PageElementIndex> selectedPageIndices = pageSelectionModel
		...
		for (int i = 0; i < selectedPageIndices.size(); i++) {
			...
			PageElement selectedPageElement = paginationModel
					.getPageElement(pageElementIndex);
			if (selectedPageElement == null) {
				continue;
			}
			Object userObjectOfSelectedPageElement = selectedPageElement
					.getUserObject();
	 ...
	 */
	public void addPageSelectionListener(
	 ...
	 * @author chetan_bh
	 */
	private class JGroupActionPanel extends Cab2bPanel implements
			ActionListener {
		private static final long serialVersionUID = 1L;
		...
		public JGroupActionPanel() {
			this(PaginationConstants.SELECT_ALL_TEXT,
					PaginationConstants.CLEAR_ALL_TEXT,
		...
		public JGroupActionPanel(String selectAllText, String clearAllText,
			...
			if (invertSelectionText != null
					&& !(invertSelectionText.trim().equals(""))) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JXPanel currentPagePanel = getPagePanel(paginationModel
	...
	/**
	 * This is a component listener for page panel as well as a parent
	 * component. TODO Needs to do some more work on this.
	 * 
	 * @author chetan_bh
	 */
	class PaginationComponentListener extends ComponentAdapter {
		public void componentResized(ComponentEvent e) {
			/**
			 * There are many conditions that needs to be checked before
			 * starting rezise. 1) automaticPageResize should be enabled. 2) No
			 * page resize for last numeric pager. 3) No page resize for
			 * non-numeric level-1 pager.
			 */
			// For Condition 1.
			...
			// For Condition 3.
			if (!paginationModel.getPagerName().equals(
					PaginationConstants.NUMERIC_PAGER)) {
				return;
			}
			...
			/*
			 * On an approximate page elements need 47 pixels each; <<-- this is
			 * a kind of assumption which may not be correct all the time. TODO
			 * Need to find proper solution.
			 */
			int difference = Math.round(Math.round(pagePanelSize.getHeight()
					- pagePanelPreferredSize.getHeight()));
	 ...
	 */
	/**
	 * Implementing {@link MouseWheelListener#mouseWheelMoved(MouseWheelEvent)}
	 * 
	 * @param e
	 * 
	 */
	public void mouseWheelMoved(MouseWheelEvent e) {
		/*
		 * Return without doing nothing, if the mouse wheel support is not
		 * enabled for JPagination.
		 */
		...
		if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
			...
			if (wheelRotation == 1) {/*
									 * If mouse wheel is rotated down/toward the
									 * user. -> move behind, decrease the page
									 * index.
									 */
				currentPageIndex = pageBar.sequentialNavigation(pageBar
						.getPreviousPageText());
			} else if (wheelRotation == -1) {/*
											 * If mouse wheel is rotated up/away
											 * from the user. -> move forward,
	...
	/**
	 * @param selectedJPageElement
	 *            the selectedJPageElement to set
	 */
