	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (session.getAttribute(Constants.SEARCH_RESULTS) != null) {
			...
			if (queryName != null) {
				...
				selectedQueryName = queryName.split(" \\(")[0];

				searchResults = (Map<ICab2bQuery, TransformedResultObjectWithContactInfo>) session
				...
				for (ICab2bQuery queryObj : queries) {
					...
					if (!failedServiceURLs.isEmpty()) {
						for (FailedTargetURL failedurl : failedServiceURLs) {
							...
							try {
								serviceurl = new ServiceURLOperations().getServiceURLbyURLLocation(failedurl
										.getTargetUrl());
		...
		if (request.getSession().getAttribute(Constants.SEARCH_RESULTS_VIEW) != null) {
			request.setAttribute(Constants.SEARCH_RESULTS_VIEW, session.getAttribute(Constants.SEARCH_RESULTS_VIEW));
		}
