...
public abstract class AbstractCaArrayResultTransfomer<R extends ICaArrayRecord>
    ...
    protected CaArrayResultTransformerUtil transformerUtil = new CaArrayResultTransformerUtil(
            super.queryLogger);

    @Override
    protected List<R> createRecords(String url,
        ...
        for (Object objRec : transformerUtil.getObjectsFromCQLResults(
    ...
    protected R createRecordForObject(String url, Object objRec,
        ...
        R rec = createCaArrayRecord(
                                    new HashSet<AttributeInterface>(attributes),
                                    id);
        ...
        for (int i = 0; i < attributes.size(); i++) {
            rec.putValueForAttribute(attributes.get(i),
    ...
    private void processAssociation(R rec, AssociationInterface association,
        ...
        for (Object associatedObj : getAssociatedObjects(desiredType,
                                                         associationValue)) {
            if (isIdentifiable(associatedObj.getClass())) {
                associatedIds.add(executeMethods(associatedObj,
                                                 GET_ID_METHOD_NAME)[0].toString());
            }
        }
        if (!associatedIds.isEmpty()) {
            rec.getAssociatedClassesIdentifiers().put(association,
                                                      associatedIds);
        }
    }

    private List<Object> getAssociatedObjects(Class<?> desiredType,
                                              Object associationValue) {
     ...
     */
    private String[] getAttributeGettersNames(
    ...
    private String[] getAssociationGettersNames(
    ...
    protected abstract R createCaArrayRecord(
                                             Set<AttributeInterface> attributes,
                                             String id);

    // /////////////////////////////////////////////////////////////////////////////////////////////
    @Override
    protected ICaArrayCategoryRecord createCategoryRecord(
