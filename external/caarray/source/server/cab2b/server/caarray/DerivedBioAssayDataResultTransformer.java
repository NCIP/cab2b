...
import cab2b.common.caarray.IDerivedBioAssayRecord;
...
public class DerivedBioAssayDataResultTransformer
        extends
        AbstractCaArrayResultTransfomer<IDerivedBioAssayRecord> {
    private static final String HEADER_ATTRIBUTE_NAME = "name";

    @Override
    protected IDerivedBioAssayRecord createCaArrayRecord(
                                                         Set<AttributeInterface> attributes,
                                                         String id) {
        return new DerivedBioAssayRecord(attributes, id);
    }

    @Override
    protected IDerivedBioAssayRecord createRecordForObject(
                                                           String url,
                                                           Object objRec,
        ...
        DerivedBioAssayRecord rec = (DerivedBioAssayRecord) super.createRecordForObject(
                                                                                        url,
                                                                                        objRec,
                                                                                        outputEntity);
        DerivedBioAssayData derivedBioAssayData = (DerivedBioAssayData) objRec;
        String[] bioAssayNames = getBioAssaysNames(
                                                   derivedBioAssayData.getBioAssayDimension().getIdentifier(),
                                                   url);
    ...
    private Object[][][] transformCubeToBQD(BioDataCube bioDataCube) {
        ...
        return (Object[][][]) MatrixDimensionSwapper.swapDimensions(
                                                                    bioDataCube.getCube(),
                                                                    dimensionMap);
    }
    ...
    private String[] getBioAssaysNames(String bioAssayDimensionId, String url) {
        BioAssayDimension bioAssayDimension = getBioAssayDimensionById(
                                                                       bioAssayDimensionId,
                                                                       url);
        return queryForNames(url, getIds(bioAssayDimension.getBioAssays()),
                             BioAssay.class.getName());
    }

    private BioAssayDimension getBioAssayDimensionById(String identifier,
                                                       String url) {
        return transformerUtil.getObjectById(BioAssayDimension.class,
                                             identifier, url);
    }
    ...
    private String[] getQuantitationTypesNames(
                                               String quantitationTypeDimensionId,
                                               String url) {
    ...
    private QuantitationTypeDimension getQuantitationTypeDimensionById(
                                                                       String identifier,
                                                                       String url) {
        return transformerUtil.getObjectById(QuantitationTypeDimension.class,
                                             identifier, url);
    }
    ...
    private String[] getDesignElementsNames(String designElementDimensionId,
                                            String url) {
    ...
    private CompositeSequenceDimension getCompositeSequenceDimensionById(
                                                                         String identifier,
                                                                         String url) {
        return transformerUtil.getObjectById(CompositeSequenceDimension.class,
                                             identifier, url);
    }
    ...
    private String[] queryForNames(String url, String[] ids, String className) {
        ...
        for (Map<String, String> row : results) {
            idToHeader.put(row.get(IDENTIFIER_ATTRIBUTE_NAME),
                           row.get(HEADER_ATTRIBUTE_NAME));
        }
