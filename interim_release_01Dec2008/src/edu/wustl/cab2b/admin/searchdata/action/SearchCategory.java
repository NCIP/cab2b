...
public class SearchCategory extends BaseAction {
	...
	/**
	 * @param stringToCreateQueryObject
	 *            the stringToCreateQueryObject to set
	 */
	 ...
	 *            the permissibleValuesChecked to set
	 */
	...
	/**
	 * Returns includeDescriptionChecked
	 * 
	 * @return
	 */
	public String getIncludeDescriptionChecked() {
		// setIncludeDescriptionChecked("off");
		return includeDescriptionChecked;
	}

	/**
	 * Sets includeDescriptionChecked
	 * 
	 ...
	 *            the selectedColumnNames to set
	 */
	 ...
	 *            the currentSelectedObject to set
	 */
	...
	/**
	 * @param currentSelectedNodeInTree
	 *            the currentSelectedNodeInTree to set
	 */
	...
	/**
	 * @param selectedColumnNameValueBeanList
	 *            the selectedColumnNameValueBeanList to set
	 */
	 ...
	 */
	@Override
	public String execute() throws IOException, NumberFormatException,
			CheckedException {
		...
		if (entityId != null) {
			...
			EntityInterface entity = cache.getEntityById(Long
					.parseLong(entityId));
		...
		if (currentPage != null
				&& currentPage.equalsIgnoreCase("prevToAddLimits")) {
			textfieldValue = "";
		}
		...
		if (textfieldValue != null && !textfieldValue.equals("")) {
			...
			Set<EntityInterface> entityCollection = matchedClass
					.getEntityCollection();
			...
			for (EntityInterface entity : resultList) {
				if (!Utility.isCategory(entity)) {
					...
					entitiesString.append(";" + entityName + "|" + entityId
							+ "|" + description);
				}
			}
	...
	/**
	 * Returns a int constant for radil option selected by user which represents
	 * Based on.
	 * 
	 * @param basedOnStr
	 *            String
	...
	/**
	 * Prepares the int [] for search targets from the checkbox values selected
	 * by user.
	 * 
	 * @param searchForm
	 *            action form
	...
	private int[] prepareSearchTarget() {
		String classCheckBoxChecked = getClassChecked();
		String attributeCheckBoxChecked = getAttributeChecked();
		String permissiblevaluesCheckBoxChecked = getPermissibleValuesChecked();
		String includeDescriptionChecked = getIncludeDescriptionChecked();
		...
		if (classCheckBoxChecked != null
				&& (classCheckBoxChecked.equalsIgnoreCase(onString) || classCheckBoxChecked
						.equalsIgnoreCase(trueString))) {
			if (includeDescriptionChecked != null
					&& (includeDescriptionChecked.equalsIgnoreCase(onString) || includeDescriptionChecked
							.equalsIgnoreCase(trueString))) {
				target
						.add(new Integer(
								edu.wustl.cab2b.common.util.Constants.CLASS_WITH_DESCRIPTION));
			} else {
				target.add(new Integer(
						edu.wustl.cab2b.common.util.Constants.CLASS));
			}
		}
		if (attributeCheckBoxChecked != null
				&& (attributeCheckBoxChecked.equalsIgnoreCase(onString) || attributeCheckBoxChecked
						.equalsIgnoreCase(trueString))) {
			if (includeDescriptionChecked != null
					&& (includeDescriptionChecked.equalsIgnoreCase(onString) || includeDescriptionChecked
							.equalsIgnoreCase(trueString))) {
				target
						.add(new Integer(
								edu.wustl.cab2b.common.util.Constants.ATTRIBUTE_WITH_DESCRIPTION));
			} else {
				target.add(new Integer(
						edu.wustl.cab2b.common.util.Constants.ATTRIBUTE));
			}
		}
		if (permissiblevaluesCheckBoxChecked != null
				&& (permissiblevaluesCheckBoxChecked.equalsIgnoreCase(onString) || permissiblevaluesCheckBoxChecked
						.equalsIgnoreCase(trueString))) {
			target.add(new Integer(edu.wustl.cab2b.common.util.Constants.PV));
		}
