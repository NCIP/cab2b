			...
			if(records.size()>0)
			{
              ...
              for (int i = offset.intValue(), l = Math.min(i + maxPageItems, records.size()); i < l; i++) 
              {
                ...
                if(selectedInstances!=null){
                    selected = selectedInstances.contains(metadata.getServiceURL())?"checked":"";
                ...
                }else
                {
					...
					<pg:item>                       
                  <tr>
                    <td align="center" valign="top"><input type="checkbox" name="checkedServiceInstances" <%=selected %> value="<%=metadata.getServiceURL()%>"	id="<%=metadata.getHostingResearchCenter()%>_checkbox"  onClick="deselectSelectAll();"/></td>
