...
public class CaDSRModelBizLogic {
	...
	private static final Logger logger = edu.wustl.common.util.logger.Logger
			.getLogger(CaDSRModelBizLogic.class);
	 ...
	 */
	public List<CaDSRModelDetailsBean> getProjectsDisplayDetails()
			throws RemoteException {
		...
		for (Project project : projects) {
			...
			if (projectLongName != null
					&& !isProjectPresent(project, cab2bEntityGroups)) {
	...
	private boolean isProjectPresent(Project project,
		...
		String projectName = Utility.createModelName(project.getLongName(),
				project.getVersion());
	 ...
	 * @return domain model
	 * @throws RemoteException
	 *             on invalid project name or could not get the instance of
	 *             CaDSRServiceClient
	 */
	public DomainModel getDomainModel(String projectLongName, String version)
			throws RemoteException {
		CaDSRServiceClient client = getCaDSRServiceClient();
	 ...
	 * @throws RemoteException
	 *             on error while getting caDSR service client or caDSR service
	 *             URL is incorrect
	 */
	 ...
	 * @return
	 */
	public Map<String, LoadModelResult> persistDomainModel(
		...
		for (CaDSRModelDetailsBean model : modelsToLoad) {
			...
			try {
				domainModel = getDomainModel(model.getLongName(), model
						.getVersion());
			...
			if (domainModel != null) {
				...
				try {
					PathBuilder.loadSingleModelFromParserObject(connection,
							parser, model.getLongName(), PATH_MAX_LENGTH);
				} catch (Throwable e) {
					/*
					 * TODO to return meaning message to UI in all cases.
					 * Catching Throwable to avoid showing direct stacktrace to
					 * user.
					 */
