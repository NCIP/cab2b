...
public class CaDSRModelBizLogicTest extends TestCase {
	...
	public void testGetProjectsDisplayDetails() {
		...
		if (projectDetails.size() == 0) {
			...
			assertTrue(name != null && name.isEmpty() == false);
			assertTrue(proj.getId() != null);
		}

		if (projectDetails.size() >= 3) {
			int i = projectDetails.get(0).getLongName().compareTo(
					projectDetails.get(2).getLongName());
			assertTrue(i < 0);
		}
	}

	public void testGetDomainModel() {
		CaDSRModelBizLogic modelBizLogic = new CaDSRModelBizLogic();
		DomainModel model = null;
		try {
			model = modelBizLogic.getDomainModel("GeneConnect", null);
		} catch (RemoteException e) {
			e.printStackTrace();
			assertFalse(true);
		}
...
class MockCaDSRModelBizLogic extends CaDSRModelBizLogic {
	...
	Collection<EntityGroupInterface> getCab2bEntityGroups() {
		...
		entityGroup.setName(Utility.createModelName(entityGroup.getLongName(),
				entityGroup.getVersion()));
		...
		entityGroupTwo.setName(Utility.createModelName(entityGroupTwo
				.getLongName(), entityGroupTwo.getVersion()));
		...
		entityGroupThree.setName(Utility.createModelName(entityGroupThree
				.getLongName(), entityGroupThree.getVersion()));
