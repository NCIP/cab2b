			...
			if(records.size()>0)
			{
              for (int i = offset.intValue(), l = Math.min(i + maxPageItems, records.size()); i < l; i++) 
              {
                ...
                String selected = metadata.isConfigured()?"checked":"";
					...
					<pg:item>                       
                  <tr>
                    <td align="center" valign="top"><input type="checkbox" name="checkedServiceInstances" <%=selected %> value="<%=metadata.getHostingResearchCenter()%>"	id="<%=metadata.getHostingResearchCenter()%>_checkbox"/></td>
