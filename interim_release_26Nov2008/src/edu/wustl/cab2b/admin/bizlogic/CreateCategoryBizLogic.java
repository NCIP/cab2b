...
public class CreateCategoryBizLogic {
	 ...
	 */
	public CreateCategoryBizLogic(Set<DAGNode> dagNodeSet,
			Set<DAGLink> dagLinkSet, Map<String, String> attributeNameMap) {
	 ...
	 */
	private List<InputCategorialAttribute> getInputCategorialAttribute(
			DAGNode dagNode) {
		...
		for (String attribute : attributesList) {
			...
			key.append(dagNode.getNodeId()).append(".")
					.append(attributeName[0]);
			inputCategorialAttribute.setDisplayName(attributeNameMap.get(key
					.toString()));
			AttributeInterface attributeInterface = EntityCache.getInstance()
					.getAttributeById(new Long(attributeName[1]));
	 ...
	 * @param link
	 */
	private void addNode(List<DAGNode> nodeLink, Set<DAGNode> dagNodeSet,
			DAGLink link) {
	...
	private List<InputCategorialClass> getInputCategorialClass() {
		...
		for (DAGNode dagNode : dagNodeSet) {
			...
			inputCategorialClass
					.setAttributeList(getInputCategorialAttribute(dagNode));
			if (getParentPathId(dagNode.getNodeId()) != -1L) {
				inputCategorialClass.setPathFromParent(getParentPathId(dagNode
						.getNodeId()));
			} else {
	...
	private void addChildrenToInputCategory(
			Map<DAGNode, InputCategorialClass> map) {
