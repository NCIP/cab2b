...
public class AuthenticationProcessor implements Filter {
	...
	/**
	 * This method is called only once at startup and we are setting the
	 * property file name
	 * 
	 * @param filterConfig
	...
	/**
	 * This method is called for every request to the server and used to check
	 * for a valid session also saves user URL in request which is used by the
	 * pagination logic
	 * 
	 * @param req
	 * @param res
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		if (session.getAttribute(AdminConstants.USER_OBJECT) == null) {
			...
			if (userName == null) {
				...
				request.getRequestDispatcher("/jsp/default.jsp").forward(req,
						res);
				return;
			}
			...
			UserInterface user = userOperation.getUserByName(userName);
			if (user != null
					&& ((User) user).getPassword().compareTo(password) == 0) {
