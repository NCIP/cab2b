...
import edu.wustl.cab2b.admin.util.MatchFactor;
...
public class CategoryDagPanel extends DAGPanel {
    private static final Logger logger = edu.wustl.common.util.logger.Logger.getLogger(CategoryDagPanel.class);
    ...
    public DAGNode createNode(String strToCreateQueryObject, String entityID) {
        ...
        String nodeName = Utility.getDisplayName(entity);
        StringBuffer sbuffer = new StringBuffer();
        String[] attributesList = strToCreateQueryObject.split(";");
        for (String attribute : attributesList) {
            String[] attributeName = attribute.split(":");
            sbuffer = sbuffer.append(" ").append(Utility.getFormattedString(attributeName[0])).append("\n");
        }
    ...
    public DAGNode editNode(String strToCreateQueryObject, String entityID) {
        ...
        for (String attribute : attributesList) {
            String[] attributeName = attribute.split(":");
            sbuffer = sbuffer.append(Utility.getFormattedString(attributeName[0])).append("\n");
        }
        String toolTip = sbuffer.toString();
    ...
    public String getLimitUI(int nodeId) {
        ...
        for (DAGNode dagNode : getDagNodeSet()) {
            if (dagNode.getNodeId() == nodeId) {
                entity = EntityCache.getCache().getEntityById(dagNode.getEntityId());
    ...
    private Map<String, List<IPath>> getPathList(List<DAGNode> linkedNodeList) {
        ...
        EntityInterface sourceEntity = cache.getEntityById(sourceNode.getEntityId());
        EntityInterface destinationEntity = cache.getEntityById(destinationNode.getEntityId());
    ...
    private void addPath(Set<IPath> pathSet, ICuratedPath curatedPaths, List<IPath> selectedPathList,
                         List<IPath> curatedPathList) {
    ...
    public String getPathDisplayString(IPath path) {
        ...
        for (IAssociation association : associationList) {
            ...
            if (isFirstAssociation) {
                ...
                String srcEntityName = edu.wustl.common.util.Utility.parseClassName(srcEntity.getName());
                String tarEntityName = edu.wustl.common.util.Utility.parseClassName(tarEntity.getName());
    ...
    public List<DAGLink> linkNodes(List<DAGNode> linkedNodeList, DAGLink selectedPath) {
        ...
        List<IPath> selectedList = getSelectedPaths(linkedNodeList, selectedPath);
        ...
        if (!selectedList.isEmpty()) {
            ...
            for (int k = 0; k < selectedList.size(); k++) {
                ...
                if (session.getAttribute(AdminConstants.PAGE_IDENTIFIER) != null
                    ...
                    return null;
                } else if (isViolatingTreeConstarint(sourceNode, destinationNode)) {
                    return null;
                }
    ...
    private List<IPath> getSelectedPaths(List<DAGNode> linkedNodeList, DAGLink selectedPath) {
        Map<String, List<IPath>> allPathMaps = getPathList(linkedNodeList);
     ...
     * @return boolean
     */
    private boolean isViolatingTreeConstarint(DAGNode src, DAGNode target) {
        if (!graph.containsVertex(target) || graph.containsEdge(src, target)) {
            return false;
        }
        return !(graph.getInNeighbours(target).isEmpty() && graph.getVertexPaths(target, src).isEmpty());
    }
    ...
    public boolean areMultipleRoots() {
        ...
        for (Long nodeId : sourceNodeList) {
            if (!destinationNodeList.contains(nodeId) && !multipleRootsList.contains(nodeId)) {
                multipleRootsList.add(nodeId);
            }
        }
        return (multipleRootsList.size() != 1);
    }

    /**
     * This method separate all sourceNodes and destination Nodes into different list
     * @param dagLink
     * @param sourceNodeList
     * @param destinationNodeList
     */
    private void setNodeList(DAGLink dagLink, List<Long> sourceNodeList, List<Long> destinationNodeList) {
        for (DAGNode dagNode : getDagNodeSet()) {
    ...
    public boolean isSameEntityGroup(Long sourceEntityId, Long targetEntityId) {
        AbstractEntityCache cache = EntityCache.getCache();
        EntityInterface source = cache.getEntityById(sourceEntityId);
        EntityInterface target = cache.getEntityById(targetEntityId);
        EntityGroupInterface sourceEntityGroup = Utility.getEntityGroup(source);
        EntityGroupInterface targetEntityGroup = Utility.getEntityGroup(target);
        boolean isSameEntityGroup = false;
        if (sourceEntityGroup == targetEntityGroup) {
    ...
    public DAGLink createInterModelJoin(int sourceNodeId, int targetNodeId, String attributeStr) {
        ...
        attributePair.setMatchFactor(MatchFactor.getMatchFactor(matchFactor));
    ...
    /**
     * @param dagNodeSet
     * @return This method will return a List of List of all DagLink Objects for Auto Connect .
     */
    ...
    private void setToolTipForCurate(Set<ICuratedPath> iPathSet, Set<DAGNode> dagNodeSet,
                                     Map<List<DAGLink>, String> map) {
    ...
    private String makeXML(Map<List<DAGLink>, String> mapForToolTip) {
        ...
        for (List<DAGLink> dagLinkList : mapForToolTip.keySet()) {
            ...
            for (DAGLink dagLink : dagLinkList) {
                ...
                builder.append("<pathId>").append(dagLink.getPathId()).append("</pathId>\n");
                builder.append("<sourceNodeId>").append(dagLink.getSourceNodeId()).append("</sourceNodeId>\n");
                builder.append("<targetNodeId>").append(dagLink.getDestinatioNodeId()).append("</targetNodeId>\n");
                builder.append("<toolTip>").append(handleSpecialCharacter(dagLink.getToolTip()));
                builder.append("</toolTip>\n").append("</DagLink>\n");
            }
            builder.append("<ToolTip>").append(handleSpecialCharacter(mapForToolTip.get(dagLinkList)));
            builder.append("</ToolTip>\n").append("</ICurated>\n");
        }
