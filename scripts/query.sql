...
alter table QUERY_BASE_EXPRESSION drop foreign key FKE8FA3F2FCF83E189;
...
create table QUERY (IDENTIFIER bigint not null auto_increment, QUERY_CONSTRAINTS_ID bigint unique, primary key (IDENTIFIER));
...
create table QUERY_BASE_EXPRESSION (IDENTIFIER bigint not null auto_increment, EXPR_TYPE varchar(255) not null, QUERY_CONSTRAINT_ID bigint, primary key (IDENTIFIER));
...
create table QUERY_CONDITION_VALUES (QUERY_CONDITION_ID bigint not null, VALUE varchar(255), POSITION integer not null, primary key (QUERY_CONDITION_ID, POSITION));
...
create table QUERY_EXPRESSION (IDENTIFIER bigint not null, IS_IN_VIEW bit, IS_VISIBLE bit, UI_EXPR_ID integer, QUERY_QUERY_ENTITY_ID bigint, primary key (IDENTIFIER));
...
alter table QUERY_BASE_EXPRESSION add index FKE8FA3F2FCF83E189 (QUERY_CONSTRAINT_ID), add constraint FKE8FA3F2FCF83E189 foreign key (QUERY_CONSTRAINT_ID) references QUERY_CONSTRAINTS (IDENTIFIER);
