...
<%@page import="edu.wustl.cab2b.common.user.ServiceURL"%>
...
<%
            ...
            String selection = "";
             ...
             Map<String ,List<String>> pagesURLs  = (Map<String ,List<String>>)session.getAttribute("SelectedInstanceMap");
                  ...
                  <table width="99%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td align="left"><table border="0" cellpadding="2" cellspacing="0">
                        ...
                        <tr>
                          <td>&nbsp;</td>
                          <td colspan="3" align="left"><span class="font_blk_s">(E.g.WUSTL,NCI)</span></td>
                        </tr>
                        <tr>
...
<%
			if(records.size()>0)
			{
			    String currentPage =  "" + currentPageNumber;
			    List<String> selectedInstances =  null;
	             if(pagesURLs!=null){
	      	       selectedInstances = pagesURLs.get(currentPage);
	             }
			    
              for (int i = offset.intValue(), l = Math.min(i + maxPageItems, records.size()); i < l; i++) 
              {
                ServiceURL metadata = records.get(i);
                chkboxNames =chkboxNames+"::"+metadata.getHostingCenter();
                String selected = ""; 
                if(selectedInstances!=null){
                    selected = selectedInstances.contains(metadata.getUrlLocation())?"checked":"";
                   
                }else
                {
                    selected = metadata.isConfigured()?"checked":"";
                }
					%>
					<pg:item>                       
                  <tr>
                    <td align="center" valign="top"><input type="checkbox" name="checkedServiceInstances" <%=selected %> value="<%=metadata.getUrlLocation()%>"	id="<%=metadata.getHostingCenter()%>_checkbox"  onClick="deselectSelectAll();"/></td>
                    <td ><span class="font_bl1_b"><%=metadata.getHostingCenter()%></span> <br />
                        <span class="font_blk_s">  <%=metadata.getDescription()%>&hellip;..</span></td>
                  </tr>
                  <tr align="center">
                    <td colspan="2">&nbsp;</td>
                  </tr>
...
%>                  
                     <pg:prev>
							...
							%>
							<a href="#" onclick="goToPage('<%= url  %>')" class="set4"><nobr>[<< Prev ]</nobr></a>
						</pg:prev> <pg:pages>
							<%
							...
							<%
							                                ...
							                                String url = request.getAttribute("URL")
							                                        + pageUrl.substring(pageUrl.indexOf("?"))+"&serviceName="+serviceName;
							%>
							<a href="#" onclick="goToPage('<%= url  %>')" class="set4"> <%=pageNumber%>&nbsp;|</a>
							<%
							}
							%>
						</pg:pages> <pg:next>
							<%
							                                    ...
							                                    + pageUrl.substring(pageUrl.indexOf("?"))+"&serviceName="+serviceName;
							%>
							<a href="#" onclick="goToPage('<%= url  %>')" class="set4"><nobr>[ Next >> ]</nobr></a>
						</pg:next>
<%} %>						                    
