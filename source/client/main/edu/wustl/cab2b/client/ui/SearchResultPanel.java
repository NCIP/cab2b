...
public class SearchResultPanel extends Cab2bPanel implements ActionListener {
	...
	public void performAddLimitAction(JXPanel[] componentPanel, EntityInterface entity) {
		...
		for (int j = 0; j < componentPanel.length; j++) {
			...
			AttributeInterface attribute = getAttribute(attributeCollection, panel
					.getAttributeName());
			ArrayList<String> conditionValues = panel.getValues();
			if (conditionString.compareToIgnoreCase("Between") == 0
	...
	public void performEditLimitAction(JXPanel[] componentPanel, IExpression expression) {
		...
		for (int j = 0; j < componentPanel.length; j++) {
			...
			if ((conditionString.equals("Is Null")) || conditionString.equals("Is Not Null")
				...
				final AttributeInterface attribute = getAttribute(attributeCollection, panel
						.getAttributeName());
	...
	class AttributeDetailsLinkListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			List<AttributeInterface> attributeList = new ArrayList<AttributeInterface>(
					attributeCollection);
			Object[][] tableData = new Object[attributeCollection.size()][4];
			Collections.sort(attributeList, new AttributeInterfaceComparator());

			int i = 0;
			for (AttributeInterface attribute : attributeList) {
				
				tableData[i][0] = CommonUtils.getFormattedString(attribute.getName());
				tableData[i][1] = attribute.getPublicId();

				Collection<SemanticPropertyInterface> semanticPropertyCollection = attribute
				...
				tableData[i][2] = conceptCodeList.toString();
				tableData[i++][3] = attribute.getDescription();
			}

			String[] headers = new String[] { " Attribute Name", " Public ID", " Concept Codes",
					" Description" };
			...
			JScrollPane jScrollPane = new JScrollPane(cab2bTable,
