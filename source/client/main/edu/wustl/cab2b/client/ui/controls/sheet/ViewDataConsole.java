...
class ViewDataConsole extends javax.swing.JPanel implements
	...
	void setupRowHeader() {
		...
		int totalWidth = tblRowHeader.getColumnModel().getColumnMargin()
				* tblRowHeader.getColumnCount()
				+ tblRowHeader.getColumnModel().getTotalColumnWidth();
		tblRowHeader.setPreferredScrollableViewportSize(new Dimension(
				totalWidth, 0));
	...
	public void setupToolBar() {
		// Make Tool bar replica of context menu...
		tbarMain.addSeparator(new Dimension(11, 0));
		tbarMain.add(actShowDetails).setText(
				actShowDetails.getValue(Action.NAME).toString());
		tbarMain.addSeparator(new Dimension(11, 0));
		...
		tbarMain.addSeparator(new Dimension(11, 0));
		tbarMain.add(actAddCol).setText(
				actAddCol.getValue(Action.NAME).toString());
		tbarMain.add(actShowProps).setText(
				actShowProps.getValue(Action.NAME).toString());
		tbarMain.add(actSelectAll).setText(
				actSelectAll.getValue(Action.NAME).toString());
		tbarMain.add(actExportSelecion).setText(
				actExportSelecion.getValue(Action.NAME).toString());
		tbarMain.add(actClearSelection).setText(
				actClearSelection.getValue(Action.NAME).toString());
		tbarMain.addSeparator(new Dimension(11, 0));
	...
	public void setReadOnlyDataModel(TableModel tmROData,
			ArrayList<SheetColumn> colSheetAL) {
	...
	private void initComponents() {
		...
		jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(
				new javax.swing.border.LineBorder(
						new java.awt.Color(51, 0, 51), 1, true), "Selection",
		...
		jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(
				new javax.swing.border.LineBorder(
						new java.awt.Color(51, 0, 51), 1, true),
		...
		lblReset.setBorder(javax.swing.BorderFactory
				.createEtchedBorder(javax.swing.border.EtchedBorder.RAISED));

		lblSelectAll.setBorder(javax.swing.BorderFactory
				.createEtchedBorder(javax.swing.border.EtchedBorder.RAISED));
		...
		butShowDetailsTB.setFont(butShowDetailsTB.getFont().deriveFont(
				butShowDetailsTB.getFont().getStyle() & ~java.awt.Font.BOLD));
		...
		butCopyTB.setFont(butCopyTB.getFont().deriveFont(
				butCopyTB.getFont().getStyle() & ~java.awt.Font.BOLD));
		...
		butPasteTB.setFont(butPasteTB.getFont().deriveFont(
				butPasteTB.getFont().getStyle() & ~java.awt.Font.BOLD));
		...
		butAddUserColTB.setFont(butAddUserColTB.getFont().deriveFont(
				butAddUserColTB.getFont().getStyle() & ~java.awt.Font.BOLD));
		butAddUserColTB
				.setToolTipText("New Column where you may edit contents");
		butAddUserColTB
				.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
		...
		butPropertiesTB.setFont(butPropertiesTB.getFont().deriveFont(
				butPropertiesTB.getFont().getStyle() & ~java.awt.Font.BOLD));
		butPropertiesTB
				.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
		...
		butResetTB.setFont(butResetTB.getFont().deriveFont(
				butResetTB.getFont().getStyle() & ~java.awt.Font.BOLD));
	...
	private void butRemoveExtendedColsActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_butRemoveExtendedColsActionPerformed
		// TODO add your handling code here:
	...
	private void tblDataMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tblDataMouseClicked
		...
		if (evt.getClickCount() == 2) {
			firePropertyChange(Common.EVENT_DATA_ROW_DOUBLE_CLICKED, -1,
					rowModel);
		} else {
	...
	private void tblRowHeaderMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tblRowHeaderMouseClicked
		...
		if (evt.getClickCount() == 2) {
			int rowModel = tblData.convertRowIndexToModel(tblData
					.getSelectedRow());
			firePropertyChange(Common.EVENT_HEADER_ROW_DOUBLE_CLICKED, -1,
					rowModel);
		}
	...
	void doSelectAll() {
		...
		extendSelectionsToAllColumns();
		tblData.getSelectionModel().setSelectionInterval(0,
				tblData.getRowCount() - 1);
	}

	/**
	 * Setups Table Model for the data-Table, from scratch.
	 */
	void setupCompositeDataModel(TableModel tmROData,
			ArrayList<SheetColumn> colSheetAL) {
		compositeDataModel = new CompositeTableModel(tmROData);

		// Disable Row & Column Selection Listeners ...
		for (ListSelectionListener colSL : colSelectionListeners) {
			tblData.getColumnModel().getSelectionModel()
					.removeListSelectionListener(colSL);
		}
		...
		for (ListSelectionListener colSL : colSelectionListeners) {
			tblData.getColumnModel().getSelectionModel()
					.addListSelectionListener(colSL);
		}
	...
	void doClearSelection() {
		doClearSelection(tblData.getSelectedRows(), tblData
				.getSelectedColumns());
	}
			...
			for (int rIdx = 0; rIdx < rowsIdx.length; rIdx++) {
				for (int cIdx = 0; cIdx < colsIdx.length; cIdx++) {
					...
					if (null == cellValue) {
						...
						sBuff
								.append(forceEscapeComma
										|| (strVal.indexOf(',') >= 0) ? '"' + strVal + '"'
										: strVal);
					}
				}
				sBuff.append("\n");
	...
	void addShowPropertyDialogActionListener(
			ActionListener actLsnShowPropDialogs) {
	...
	protected ImageIcon createImageIcon(String pathImage) {
		...
		if (imgURL != null) {
			...
			System.err
					.println("[ViewDataConsole]: Unable to locate Toolbar icon images: "
							+ pathImage);
			return null;
		}
	}
	...
	class ExportSelectionAction extends AbstractAction implements
		...
		private void addAssociatedTable(JTable tbl) {
			...
			tbl.getColumnModel().getSelectionModel().addListSelectionListener(
					this);
		}

		public void valueChanged(ListSelectionEvent e) {
			setEnabled(tblAssociated.getSelectedRowCount()
	...
	class RowSelectionsListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (!e.getValueIsAdjusting()) {
				...
				firePropertyChange(JSheet.EVENT_DATA_SINGLE_CLICKED, -1, e
						.getLastIndex());
			}
		}
	}

	class CompositeTableModel extends AbstractTableModel implements
			TableModelListener {
		//
		...
		private CompositeTableModel(TableModel roDataModel) {
			...
			{
				extDataModel = new DefaultTableModel(roDataModel.getRowCount(),
						0);
			}
		...
		private void setPrimaryDataModel(TableModel roDataModel) {
			if (null == roDataModel) {
				throw new IllegalStateException("Primary data Model NOT set. "
						...
						+ "setPrimaryDataModel(). NOTE: roDataModel="
						+ roDataModel);
			}
		...
		public void tableChanged(TableModelEvent oe) {
			...
			TableModelEvent ne = new TableModelEvent((TableModel) oe
					.getSource(), fr, lr, col, oe.getType());
			// Relay to Parent...
			fireTableChanged(ne);
		}
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			...
			// information...
			{
				throw new IllegalStateException(
						"These table cells are read-Only.");
			}
	...
	class FixedTblSelHangerRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			return super.getTableCellRendererComponent(table, value, isSelected
					&& isSelectionFromRowHeader, hasFocus, row, column);
		}
	}
