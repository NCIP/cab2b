...
public class ClassNodeRenderer implements IGraphNodeRenderer 
{
	 ...
	 * @param node Node for which to perform layout
	 * @param gr Graphics2D object associated with this node renderer 
	 */
	private void layoutMinimizedNode(IGraphNode node, Graphics2D gr)
	{
        ...
        m_textRect = fontMetrics.getStringBounds(((IDisplayable) node.getLookup().lookup(IDisplayable.class)).getDisplayName(), gr).getBounds();
        m_textRect.width += 10;
		...
		{
        	helper.setPortRelativeLocation(targetPortList.get(i), new Point(m_numRectangle.x - 5, m_numRectangle.y + 5));
		}
	 ...
	 * @param associationList List of associations to layout
	 * @param gr Graphics2D object associated with this node renderer
	 */
	private void layoutAssociations(IGraphNode node, Graphics2D gr)
	{
		...
		// Common condtion portion where attribute conditions will be logically ANDED / ORED will association
		// conditions
	...
	/**
     * Called to layout a node. This means to resolve the relative active areas and the relative bounds of the node. 
     * Optionally the relative location of the node ports also.
     * Use GraphHelper.setNodeRelativeBounds, GraphHelper.setNodeRelativeActiveAreas, and 
     * optionally GraphHelper.setPortRelativeLocation methods.
     * <p/>
     * Note: The node relative bounds must include the node ports bounds. 
     * This could be done by calling GraphHelper.includePortsToNodeRelativeBounds method.
     * <p/>
     * Note: The node do not have resolved location, active areas, and bounds yet.
	 ...
	 * @param node GraphNode to render
	 * @param gr Graphics2D object associated with this node renderer
	 * @param layer Layer of the node to render
	 */
	private void renderMinimizedNode(IGraphNode node, Graphics2D gr, int layer)
	{
        ...
        if (layer == LAYER_BACKGROUND)
        {
        	...
        	gr.setColor(new Color(0xFFDF57));
        ...
        else if (layer == LAYER_NODE) 
        {
           ...
           gr.fillRect(currX + textRect.width + m_maxMinLength/4, currY+3+ 3*m_maxMinLength/4, 3*m_maxMinLength/4, m_maxMinLength/4);
	...
	private void renderMaximizedNode(IGraphNode node, Graphics2D gr, int layer)
	{
        ...
        if (layer == LAYER_BACKGROUND)
        {
            ...
            if(associationList.size() > 0)
            {
        		...
        		gr.fillRect(rect.x, rect.y + m_textRect.height + m_option.height, rect.width, m_assRectangles[0].height);
        		gr.setColor(new Color(0x000000)); 
        		rect2D = new Rectangle2D.Float(rect.x, rect.y + m_textRect.height + m_option.height, rect.width-1, m_assRectangles[0].height);
        		gr.draw(rect2D);
        		rect2D = new Rectangle2D.Float(rect.x+1, rect.y + m_textRect.height + m_option.height+1, rect.width-2, m_assRectangles[0].height-2);
        		gr.draw(rect2D);       		
        		int currY = rect.y + m_textRect.height + m_option.height;
            	for(int i=1; i<m_assRectangles.length; i++)
            	{
            		gr.setColor(new Color(0xF5F5F5));
            		gr.fillRect(rect.x, currY + (i * m_assRectangles[i-1].height), rect.width, m_assRectangles[i].height);
            		gr.setColor(new Color(0x000000)); 
            		rect2D = new Rectangle2D.Float(rect.x, currY + (i * m_assRectangles[i-1].height), rect.width-1, m_assRectangles[i].height);
            		gr.draw(rect2D);
            	}
            }
	 ...
	 * @param gr Graphics2D object associated with this node renderer
	 * @param currX Current X location where association box has to render
	 ...
	 * @param str String to draw in drop downn
	 * @param x X location for drop-down
	 * @param y Y location for drop-down 
	 * @param gr Graphics2D object associated with this node renderer 
	 */
	...
	/**
	 * 
	 * Called to render a layer of a node. This method is always called after layoutNode method.
	 *
	 */
	 ...
	 * @param node The node for which to check the click
	...
	/**
	 * Method to check if maxmin rectangle has been clicked. This checks wheather to
	 * minimize or maximize the node in selection
	 * @param node Node for which to check max-min rectangle 
	 * @param point Point location of mouse click
	 * @return
	 */
	 ...
	 * @param node Node for which to check if association dropdown is checked
	 * @param point where mouse has been clicked
