...
public class MainDagPanel extends Cab2bPanel {
	 ...
	 * @param pathFinder
	 */
	public MainDagPanel(IUpdateAddLimitUIInterface addLimitPanel,
			Map<DagImages, Image> dagImageMap, IPathFinder pathFinder, boolean isDAGForView,
	...
	private void initGUI() {
		...
		m_view = GraphFactory.createView(m_document, m_documentRenderer, m_viewController,
				m_eventHandler);
    ...
    private List<IAssociation> addAssociations(IConstraints constraints, IJoinGraph joinGraph,
        ...
        for (int index = 0; index < childSize; index++) {
            ...
            EntityInterface source = constraints.getExpression(parentExpressionId).getQueryEntity().getDynamicExtensionsEntity();
	...
	public void updateGraph(IExpressionId expressionId) throws MultipleRootsException {
		...
		node.setDisplayName(entityNameDisplayer.getEntityDisplayName(constraintEntity
	...
	public void updateGraphForViewExpression(IExpressionId expressionId)
			throws MultipleRootsException {
		updateGraph(expressionId);
	}
	...
	private boolean isLinkingValid(List<ClassNode> selectedNodes) {
		...
		if (m_currentNodeList.size() < 2 || selectedNodes.size() != 2 || selectedNodes == null) {
			...
			JOptionPane.showMessageDialog(this, errorMessage, "Connect Nodes warning",
	...
	protected List<IPath> getPaths(ClassNode sourceNode, ClassNode destNode) {
		...
		AmbiguityObject ambiguityObject = new AmbiguityObject(sourceEntity
				.getDynamicExtensionsEntity(), destinationEntity.getDynamicExtensionsEntity());
	...
	protected void linkNode(final ClassNode sourceNode, final ClassNode destNode) {
		...
		if (!m_queryObject.isPathCreatesCyclicGraph(sourceNode.getExpressionId(), destNode
				.getExpressionId(), paths.get(0))) {
	 ...
	 */
	protected void LinkTwoNode(final ClassNode sourceNode, final ClassNode destNode,
			final IPath path, List<IExpressionId> intermediateExpressions,
			final boolean updateQueryRequired) {
		...
		if (updateQueryRequired) {
			try {
				...
				JOptionPane.showMessageDialog(this,
						"Cannot  connect nodes as it creates cycle in graph",
						"Connect Nodes warning", JOptionPane.WARNING_MESSAGE);
				return;
			}
		}
		...
		if (updateQueryRequired) {
			...
			if (assPosition != 0) {
				...
				ILogicalConnector logicalConnector = sourceExp.getLogicalConnector(sourceExp
						.indexOfOperand(nextExpId) - 1, sourceExp.indexOfOperand(nextExpId));
	...
	private void updateQueryObject(PathLink link, ClassNode sourceNode, ClassNode destNode,
		...
		m_queryObject.setLogicalConnector(sourceNode.getExpressionId(), destId, Utility
				.getLogicalOperator(operator), false);

		// Put appropriate parenthesis
		if (sourcePort != null) {
			IExpressionId previousExpId = sourceNode.getLinkForSourcePort(
					sourceNode.getSourcePortAt(0)).getLogicalConnectorExpressionId();
	...
	public void deletePath(PathLink link) {
		...
		if (0 == expressionIds.size()) {
			m_queryObject.removeAssociation(link.getSourceExpressionId(), link
	 ...
	 */
	public void updateLogicalOperatorForAssociation(ClassNode sourceNode, PathLink link,
			String operator) {
		IExpressionId destinationId = link.getLogicalConnectorExpressionId();
		m_queryObject.setLogicalConnector(sourceNode.getExpressionId(), destinationId, Utility
				.getLogicalOperator(operator), true);
		m_expressionPanel.setText(getExprssionString());
	}
	...
	private String FormExpression(HashMap<IExpressionId, String> expressionToStringMap,
		...
		if (ports.size() > 0) {
			...
			for (int i = 0; i < ports.size(); i++) {
				IExpressionId associationNode = node.getLinkForSourcePort(ports.get(i))
						.getDestinationExpressionId();
				if (i > 0 && !getNodeType(associationNode).equals(ClassNodeType.ViewOnlyNode)) {
					expressionString.append(" ").append(node.getLogicalOperator(ports.get(i)))
							.append(" ");
				}
				...
				if (expressionToStringMap.get(associationNode) != null) {
					...
					expressionString.append(FormExpression(expressionToStringMap,
							expressionsCovered, getNode(associationNode)));
				}
	...
	protected void autoConnectNodes(List<ClassNode> selectedNodes, ICuratedPath curatedPath) {
		...
		for (IPath path : paths) {
			...
			List<ClassNode> destinationNodes = getNodesWithEntity(selectedNodes, path
					.getTargetEntity());
			for (int i = 0; i < sourceNodes.size(); i++) {
				for (int j = 0; j < destinationNodes.size(); j++) {
					LinkTwoNode(sourceNodes.get(i), destinationNodes.get(j), path,
							new ArrayList<IExpressionId>(), true);
				}
			}
		}
		m_expressionPanel.setText(getExprssionString());
	...
	private boolean isAutoConnectValid(List<ClassNode> selectedNodes) {
		for (int i = 0; i < selectedNodes.size(); i++) {
			...
			for (int j = 0; j < ports.size(); j++) {
				IExpressionId id = sourceNode.getLinkForSourcePort(ports.get(j))
						.getDestinationExpressionId();
	...
	private List<ClassNode> getNodesWithEntity(List<ClassNode> classNodes, EntityInterface entity) {
		...
		for (int i = 0; i < classNodes.size(); i++) {
			...
			EntityInterface currentEntity = expression.getQueryEntity()
					.getDynamicExtensionsEntity();
