...
public class ViewController extends DefaultViewController implements ActionListener {
	...
	protected void deleteNode(IGraphNode node) {
		try {
			IExpressionId expressionId = ((ClassNode) node.getLookup().lookup(ClassNode.class))
					.getExpressionId();
	...
	private void deleteLink(IGraphLink link) {
		...
		ClassNode destinationNode = (ClassNode) targetPort.getNode().getLookup().lookup(
				ClassNode.class);
		if (m_mainPanel.isDAGForView()) {
		...
		if (graphDocument != null) {
			...
			if (event != null) {
				...
				for (int j = 0; j < selectedNodes.length; j++) {
					ClassNode tempNode = (ClassNode) selectedNodes[j].getLookup().lookup(
							ClassNode.class);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == m_andOperator) {
			ClassNode classNode = (ClassNode) m_currentNode.getLookup().lookup(ClassNode.class);
			ClassNodeRenderer nodeRenderer = (ClassNodeRenderer) getHelper().getNodeRenderer(
					m_currentNode);
			...
			// changed
			{
				...
				m_mainPanel.updateLogicalOperatorForAttributes(classNode,
						ClientConstants.OPERATOR_AND);
			} else {
				classNode.setLogicalOperator(index, ClientConstants.OPERATOR_AND);
				m_mainPanel.updateLogicalOperatorForAssociation(classNode, classNode
						.getLinkForSourcePort(classNode.getSourcePortAt(index)),
						ClientConstants.OPERATOR_AND);
			}
			getHelper().scheduleNodeToLayout(m_currentNode);
			getHelper().recalculate();
