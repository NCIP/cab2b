...
public class ExperimentDataCategoryGridPanel extends Cab2bPanel {
	 ...
	 */
	public void setCurrentSpreadSheetViewPanel(
			DefaultSpreadSheetViewPanel currentSpreadSheetViewPanel) {
	...
	public void refreshTable(List<IRecord> recordList) {
		...
		this.spreadSheetViewPanel.getDataTable().addFocusListener(
				new TableFocusListener(this.experimentPanel));
	...
	public void initGUI() {
		...
		spreadSheetViewPanel = new DefaultSpreadSheetViewPanel(true, false,
				new ArrayList<IRecord>(), true);
	...
	final private void addAnalysisViewTabPanel(UserObjectWrapper<List<IRecord>> userObjectWrapper) {
		...
		if (analysisView == null) {
			...
			DefaultSpreadSheetViewPanel defaultSpreadSheetViewPanel = new DefaultSpreadSheetViewPanel(
					true, false, recordList, true);
	...
	public void saveDataCategory(final String title) {
		...
		CustomSwingWorker swingWorker = new CustomSwingWorker(MainFrame.openExperimentWelcomePanel) {
			...
			protected void doNonUILogic() throws RuntimeException {
				...
				ExperimentBusinessInterface ExperimentBI = (ExperimentBusinessInterface) CommonUtils
				...
				try {
					dataListMetadata = dataListBI.saveDataList(customCategoryDataList
							.getRootDataRow(), dataListMetadata);

					experimentPanel.getSelectedExperiment().addDataListMetadata(dataListMetadata);

					ExperimentBI.addDataListToExperiment(experimentPanel.getSelectedExperiment()
							.getId(), dataListMetadata.getId());
	...
	class TableLinkAction extends LinkAction {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			final UserObjectWrapper<List<IRecord>> userObjectWrapper = (UserObjectWrapper<List<IRecord>>) analysisTable
			...
			if (selectedTabPanel != null && selectedTabPanel.getComponentCount() > 0) {
				...
				if (component instanceof DefaultSpreadSheetViewPanel) {
					experimentDataCategoryGridPanel
							.setCurrentSpreadSheetViewPanel((DefaultSpreadSheetViewPanel) component);
