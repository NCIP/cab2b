...
public class ExperimentOpenPanel extends Cab2bTitledPanel {
	...
	public ExperimentOpenPanel(ExperimentTreeNode Obj,
			ExperimentDetailsPanel parentPanel) {
	...
	public void initGUI() {
		...
		try {
			selectedExperiment = expBus.getExperiment(m_ExperimentTreeNodeObj
					.getIdentifier());
		} catch (RemoteException e) {
			CheckedException checkedException = new CheckedException(e
					.getMessage());
			CommonUtils.handleException(checkedException, m_parentPanel, true,
					true, true, false);
			e.printStackTrace();
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				experimentStackBox, experimentDataCategoryGridPanel);
