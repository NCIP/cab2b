...
public class ExperimentStackBox extends Cab2bPanel {
	...
	private DefaultMutableTreeNode generateRootNode(Experiment experiment) {
		...
		for (DataListMetadata dataListMetadata : experiment.getDataListMetadataCollection()) {
			...
			for (IdName idName : dataListMetadata.getEntitiesNames()) {
				...
				EntityInterface originalEntity = clientCache.getEntityById(idName
						.getOriginalEntityId());
	...
	public void treeSelectionListenerAction() {
		...
		Cab2bPanel selectedPanel = (Cab2bPanel) m_experimentDataCategoryGridPanel.getTabComponent()
		...
		CustomSwingWorker swingWorker = new CustomSwingWorker(datalistTree) {
			...
			protected void doNonUILogic() throws RuntimeException {
				try {
					...
					CommonUtils.handleException(remoteException, MainFrame.newWelcomePanel, true,
							true, true, false);
				}
	...
	private void showChartAction(ChartType chartType) {
		...
		if (currentChartPanel == null) {
			...
			DefaultSpreadSheetViewPanel defaultSpreadSheetViewPanel = m_experimentDataCategoryGridPanel
					.getCurrentSpreadSheetViewPanel();
			Cab2bChartPanel cab2bChartPanel = new Cab2bChartPanel(defaultSpreadSheetViewPanel
					.getDataTable());
			...
			Cab2bChartPanel cab2bChartPanel = (Cab2bChartPanel) CommonUtils.getComponentByName(
					currentChartPanel, "cab2bChartPanel");
			cab2bChartPanel.setChartType(chartType, entityName);
	...
	private void addAvailableAnalysisServices(IdName idName) {
		...
		try {
			serviceDetailInterfaceList = analyticalServiceOperationsBusinessInterface
					.getApplicableAnalyticalServices(idName.getOriginalEntityId());
	...
	private void serviceSelectAction(ServiceDetailsInterface serviceDetails,
		...
		finishButton.addActionListener(new FinishButtonActionListner(serviceDetails, dataEntity,
				requiredEntity, m_experimentDataCategoryGridPanel));
...
class FinishButtonActionListner implements ActionListener {
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		// Obtain table data
		DefaultSpreadSheetViewPanel defaultSpreadSheetViewPanel = experimentDataCategoryGridPanel
				.getCurrentSpreadSheetViewPanel();
		...
		AnalyticalServiceOperationsBusinessInterface analyticalServiceOperationsBusinessInterface = (AnalyticalServiceOperationsBusinessInterface) CommonUtils
				.getBusinessInterface(EjbNamesConstants.ANALYTICAL_SERVICE_BEAN,
						AnalyticalServiceOperationsHomeInterface.class, null);
		try {
			entityRecordList = analyticalServiceOperationsBusinessInterface.invokeService(
					serviceDetails, recordList, serviceParameterList);
	...
	private List<IRecord> collectServiceParameterValues(final Component[] controlPanels) {
		List<AttributeInterface> attributeList = new ArrayList<AttributeInterface>(requiredEntity
				.getAttributeCollection());
		Collections.sort(attributeList, new AttributeInterfaceComparator());

		IRecord record = QueryResultFactory.createRecord(new HashSet<AttributeInterface>(
				attributeList), new RecordId("-1", ""));
