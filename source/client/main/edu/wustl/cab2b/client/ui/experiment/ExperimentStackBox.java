...
public class ExperimentStackBox extends Cab2bPanel {
	...
	private DefaultMutableTreeNode generateRootNode(Experiment experiment) {
		...
		DefaultMutableTreeNode CategoriesRoot = new DefaultMutableTreeNode(
				"Experiment Data Categories");
		DefaultMutableTreeNode customCategoriesRoot = null;

		// for each datalist
		for (DataListMetadata dataListMetadata : experiment
				.getDataListMetadataCollection()) {

			if (dataListMetadata.isCustomDataCategory()) {
				if (customCategoriesRoot == null) {
					customCategoriesRoot = new DefaultMutableTreeNode(
			...
			for (IdName idName : dataListMetadata.getEntitiesNames()) {
				...
				EntityInterface originalEntity = clientCache
						.getEntityById(idName.getOriginalEntityId());

				UserObjectWrapper<IdName> categoryUserObject = new UserObjectWrapper<IdName>(
						idName, Utility.getDisplayName(originalEntity));
	...
	private void initializeDataListTree(DefaultMutableTreeNode rootNode) {
		...
		datalistTree.setOpenIcon(new ImageIcon(loader
				.getResource(TREE_OPEN_FOLDER)));
		datalistTree.setClosedIcon(new ImageIcon(loader
				.getResource(TREE_CLOSE_FOLDER)));
		datalistTree.setLeafIcon(new ImageIcon(loader
				.getResource(TREE_LEAF_NODE)));
		...
		customCategoryButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					customDataCategoryModel = expBus
							.getDataCategoryModel(m_selectedExperiment);
				...
				CustomCategoryPanel categoryPanel = new CustomCategoryPanel(
						customDataCategoryModel, expBus, m_selectedExperiment);
	...
	private void initializePanels() {
		...
		stackedBox
				.addBox("Analyze Data ", analyseDataPanel, ANALYZE_DATA, true);
		...
		stackedBox.setBorder(BorderFactory.createLineBorder(new Color(200, 200,
				220)));
		this.add(stackedBox);
		updateUI();
	}
	...
	public void treeSelectionListenerAction() {
		...
		Component applyFilterComponent = CommonUtils.getComponentByName(
				selectedPanel, Constants.APPLY_FILTER_PANEL_NAME);
		...
		CustomSwingWorker swingWorker = new CustomSwingWorker(this) {
			...
			protected void doNonUILogic() throws RuntimeException {
				try {
					...
					recordList = dataListBI.getEntityRecord(idName
							.getUserObject().getId());
	...
	public static void setDataForFilterPanel(Vector<CaB2BFilterInterface> data) {
		...
		for (CaB2BFilterInterface caB2BFilterInterface : data) {
			...
			if ((labelName.indexOf("like **") < 0)
					&& (labelName.indexOf("like *.*") < 0)) {
	...
	private void setBarChartLink() {
		URL url = this.getClass().getClassLoader().getResource(
				ClientConstants.BAR_GRAPH);
		setVisulaizationToolLink(ClientConstants.BAR_CHART, url,
				new LinkActionListener(ClientConstants.BAR_CHART));
	}

	private void setLineChartLink() {
		URL url = this.getClass().getClassLoader().getResource(
				ClientConstants.LINE_GRAPH);
		setVisulaizationToolLink(ClientConstants.LINE_CHART, url,
				new LinkActionListener(ClientConstants.LINE_CHART));
	}

	private void setScatterPlotLink() {
		URL url = this.getClass().getClassLoader().getResource(
				ClientConstants.SCATTER_GRAPH);
		setVisulaizationToolLink(ClientConstants.SCATTER_PLOT, url,
				new LinkActionListener(ClientConstants.SCATTER_PLOT));
	}

	private void setHeatmapLink() {
		URL url = this.getClass().getClassLoader().getResource(
				ClientConstants.HEAT_MAP);
		setVisulaizationToolLink(ClientConstants.HEATMAP, url,
				new LinkActionListener(ClientConstants.HEATMAP));
	}
	 ...
	 */
	private void setVisulaizationToolLink(String toolType, URL iconUrl,
	...
	private class LinkActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			CustomSwingWorker customSwingWorker = new CustomSwingWorker(
					ExperimentStackBox.this) {
				@Override
	...
	private void showChartAction(String chartType) {
		...
		if (currentChartPanel == null) {
			...
			DataListDetailedPanelInterface dataListDetailedPanel = m_experimentDataCategoryGridPanel
					.getCurrentSpreadSheetViewPanel();
			Cab2bChartPanel cab2bChartPanel = new Cab2bChartPanel(
					dataListDetailedPanel, entityName);
			...
			m_experimentDataCategoryGridPanel
					.setCurrentChartPanel(visualizationTabPanel);
			tabComponent.add("Chart" + ++chartIndex, visualizationTabPanel);
			tabComponent.setSelectedComponent(visualizationTabPanel);
		} else {
			Cab2bChartPanel cab2bChartPanel = (Cab2bChartPanel) CommonUtils
					.getComponentByName(currentChartPanel, "cab2bChartPanel");
			cab2bChartPanel.drawChart(chartType);
			tabComponent.setSelectedComponent(currentChartPanel);
		}
	}

	private Cab2bPanel createVisualizationTabPanel(
			final JTabbedPane tabComponent) {
	...
	private void showHeatmapAction() {
		final JTabbedPane tabComponent = m_experimentDataCategoryGridPanel
				.getTabComponent();
		Cab2bPanel currentHeatMapPanel = m_experimentDataCategoryGridPanel
				.getCurrentHeatMapPanel();
	...
	public void updateStackBox(DataListMetadata dataListMetadata) {
		...
		if (rootNode.getChildCount() == 1) {
			...
			customCategoriesRoot = (DefaultMutableTreeNode) rootNode
					.getChildAt(CUSTOM_CATEGORY_NODE_NO);
		}
	...
	DefaultMutableTreeNode getNodeForCustomCategory(
		...
		UserObjectWrapper<IdName> customCategoryUserObject = new UserObjectWrapper<IdName>(
				idName, dataListMetadata.getName());
	...
	private void addAvailableAnalysisServices(IdName idName) {
		...
		EntityInterface dataEntity = clientSideCache.getEntityById(idName
				.getOriginalEntityId());
		...
		try {
			serviceDetailInterfaceList = analyticalServiceOperationsBusinessInterface
					.getApplicableAnalyticalServices(idName
							.getOriginalEntityId());
	...
	private void addHyperLinkToAnalyticalPane(
		...
		hyperlink.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent actionEvent) {
				Cab2bHyperlink<ServiceDetailsInterface> hyperlink = (Cab2bHyperlink<ServiceDetailsInterface>) (actionEvent
						.getSource());

				ServiceDetailsInterface serviceDetails = hyperlink
						.getUserObject();
	...
	private void serviceSelectAction(ServiceDetailsInterface serviceDetails,
		...
		finishButton.addActionListener(new FinishButtonActionListner(
				serviceDetails, dataEntity, requiredEntity,
				m_experimentDataCategoryGridPanel));
		...
		try {
			...
			if (attributeCollection != null) {
				...
				Collections.sort(attributeList,
						new AttributeInterfaceComparator());

				Dimension maxLabelDimension = CommonUtils
						.getMaximumLabelDimension(attributeList);
				for (AttributeInterface attribute : attributeList) {
					AbstractTypePanel jxPanel = (AbstractTypePanel) SwingUIManager
			...
			CommonUtils.handleException(checkedException, this, true, false,
					false, false);
		}
...
class FinishButtonActionListner implements ActionListener {
	...
	public FinishButtonActionListner(ServiceDetailsInterface serviceDetails,
			EntityInterface dataEntity, EntityInterface requiredEntity,
			ExperimentDataCategoryGridPanel experimentDataCategoryGridPanel) {
		super();
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		List<IRecord> serviceParameterList = collectServiceParameterValues(parameterPanel
				.getComponents());

		// Obtain table data
		DataListDetailedPanelInterface dataListDetailedPanelInterface = experimentDataCategoryGridPanel
				.getCurrentSpreadSheetViewPanel();
		if (dataListDetailedPanelInterface instanceof DefaultSpreadSheetViewPanel) {
			...
			List<IRecord> recordList = defaultSpreadSheetViewPanel
					.getSelectedRecords();
			...
			AnalyticalServiceOperationsBusinessInterface analyticalServiceOperationsBusinessInterface = (AnalyticalServiceOperationsBusinessInterface) CommonUtils
					.getBusinessInterface(
							...
							null);
			try {
				entityRecordList = analyticalServiceOperationsBusinessInterface
						.invokeService(serviceDetails, recordList,
	...
	private void updateAnalysisTable(final String analysisTitle,
			final List<IRecord> entityRecordList) {
	 ...
	 */
	private List<IRecord> collectServiceParameterValues(
		...
		IRecord record = QueryResultFactory.createRecord(
				new HashSet<AttributeInterface>(attributeList), new RecordId(
						"-1", ""));

		int index = 0;
		for (AttributeInterface attribute : attributeList) {
			String value = null;
			ArrayList<String> parameterValues = ((IComponent) controlPanels[index++])
					.getValues();
