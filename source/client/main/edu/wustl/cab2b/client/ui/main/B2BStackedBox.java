...
public class B2BStackedBox extends Cab2bPanel {
	...
	private void initGUI() {
		...
		Cab2bLabel myCategoryLabel = new Cab2bLabel(
				"This will be available in next release");
		Cab2bPanel myCategories = new Cab2bPanel();
		myCategories.add(myCategoryLabel);
		final String titleMyCategories = ApplicationProperties
				.getValue(CATEGORY_BOX_TEXT);
		box.addBox(titleMyCategories, myCategories, MY_CATEGORIES_IMAGE, false);

		Cab2bLabel savedQueryLabel = new Cab2bLabel(
				"This will be available in next release");
		Cab2bPanel savedQueries = new Cab2bPanel();
		savedQueries.add(savedQueryLabel);
		final String titleQuery = ApplicationProperties
				.getValue(QUERY_BOX_TEXT);
		box.addBox(titleQuery, savedQueries, MY_SEARCH_QUERIES_IMAGE, false);

		Cab2bLabel popularCategoryLabel = new Cab2bLabel(
				"This will be available in next release");
		Cab2bPanel popularCategories = new Cab2bPanel();
		popularCategories.add(popularCategoryLabel);
		final String titlePopularcategories = ApplicationProperties
				.getValue(POPULAR_CATEGORY_BOX_TEXT);
		box.addBox(titlePopularcategories, popularCategories,
				POPULAR_CATEGORIES_IMAGE, false);
	...
	class ShowMessageActionClass implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JOptionPane.showMessageDialog(comp, msg, "caB2B Information",
	 ...
	 * @param data
	 */
	private void setDataForPanel(JPanel panel,
			Collection<IParameterizedQuery> data) {
