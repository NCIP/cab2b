...
public class ParameterizedQueryConditionPanel extends Cab2bTitledPanel {
	...
	public ParameterizedQueryConditionPanel(
			ParameterizedQueryDataModel queryDataModel, boolean viewAllAttribute) {
		super();
	...
	private Dimension getMaximumDimensionForAttribute() {
		Map<Integer, Collection<AttributeInterface>> allAttributes = queryDataModel
				.getAllAttributes();
		List<AttributeInterface> attributeList = new ArrayList<AttributeInterface>();
		for (Integer exprId : allAttributes.keySet()) {
			Collection<AttributeInterface> attributeCollection = allAttributes
					.get(exprId);
		...
		if (definedAttributePanel != null) {
			...
			for (int index = 0; index < panelCount; index++) {
				if (definedAttributePanel.getComponent(index) instanceof AbstractTypePanel) {
					AbstractTypePanel conditionPanel = (AbstractTypePanel) definedAttributePanel
							.getComponent(index);

					if (conditionPanel.getAttributeEntity() == componentPanel
							.getAttributeEntity()
							&& conditionPanel.getExpressionId() == componentPanel
									.getExpressionId()) {
						componentPanel.setAttributeCheckBox(conditionPanel
								.isAttributeCheckBoxSelected());
						componentPanel.setAttributeDisplayName(conditionPanel
								.getAttributeDisplayName());
						componentPanel.setCondition(conditionPanel
								.getConditionItem());
						componentPanel.setValues(new ArrayList<String>(
								conditionPanel.getValues()));
					}
				}
			}
		}
			...
			for (Integer exprId : allAttributes.keySet()) {
				for (AttributeInterface attribute : allAttributes.get(exprId)) {
					if (!allConditionAttributeList.contains(attribute)) {
						AbstractTypePanel componentPanel = (AbstractTypePanel) SwingUIManager
						...
						componentPanel.setAttributeDisplayName(exprId
								.intValue()
								+ "."
	...
	private void setConditionValues(AbstractTypePanel componentPanel) {
		...
		Map<Integer, Collection<ICondition>> conditionMap = queryDataModel
				.getConditions();
		for (Integer key : conditionMap.keySet()) {
			for (ICondition condition : conditionMap.get(key)) {
				if (condition.getAttribute() == attribute) {
					componentPanel.setValues(new ArrayList<String>(condition
							.getValues()));
					componentPanel.setCondition(condition
	...
	private void showOnlyConditionPanel() {
		...
		try {
			...
			for (Integer key : conditionMap.keySet()) {
				for (ICondition condition : conditionMap.get(key)) {
					if (!allConditionAttributeList.contains(condition
							.getAttribute())) {
						componentPanel = (AbstractTypePanel) SwingUIManager
								.generateUIPanel(parseFile, condition
										.getAttribute(), maxLabelDimension);
	...
	private List<AttributeInterface> getAllConditionAttribute(
			Container basePanel) {
		...
		for (Integer index : allConditionPanel.keySet()) {
			attributeList
					.add(allConditionPanel.get(index).getAttributeEntity());
		}
	...
	private Cab2bPanel getLabelHeaderPanel() {
		...
		Font newBoldFont = new Font(userDefinedLabel.getFont().getName(),
				Font.BOLD, userDefinedLabel.getFont().getSize());
		...
		Dimension labelDimension = new Dimension(maxLabelDimension.width + 10,
				maxLabelDimension.height + 10);
		userDefinedLabel.setPreferredSize(new Dimension(80,
	...
	private void populatePanelIndexList(final Cab2bPanel cab2bPanel) {
		...
		for (int index = 0; index < cab2bPanel.getComponentCount(); index++) {
			panelIndexList.add((AbstractTypePanel) cab2bPanel
					.getComponent(index));
		}
	}
	...
	public List<AbstractTypePanel> getUnCheckedAttributePanels() {
		...
		for (int index = 0; index < basePanel.getComponentCount(); index++) {
			if (basePanel.getComponent(index) instanceof AbstractTypePanel) {
				AbstractTypePanel panel = (AbstractTypePanel) basePanel
						.getComponent(index);
	...
	public TreeMap<Integer, AbstractTypePanel> getAllConditionAttributePanels(
			Container basePanel) {
		...
		for (int index = 0; index < basePanel.getComponentCount(); index++) {
			if (basePanel.getComponent(index) instanceof AbstractTypePanel) {
				...
				if (panel.isAttributeCheckBoxSelected()
						|| conditionString.equals("Is Null")
						|| conditionString.equals("Is Not Null")
						|| panel.getValues().size() > 0) {
