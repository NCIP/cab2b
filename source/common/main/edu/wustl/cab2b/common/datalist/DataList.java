...
public class DataList implements Serializable
{
	 ...
	 */
	private void findNodeInDataList(int pos, List<IDataRow> currentDataList,
			List<IDataRow> listWithEntity, List<IDataRow> pathEnitites)
	{
	 ...
	 * @return this DataRow object from the data list.
	 */
	private void getDataRowWithEntity(int pos, List<IDataRow> currentDataList,
			List<IDataRow> listWithEntity, List<IDataRow> pathEnitites)
	{
		for (int i = 0; i < currentDataList.size(); i++)
		{
				...
				if (rootChildNodes.get(i).isData() == true)
				{
					...
					for (int j = 0; j < childNodes.size(); j++)
					{
						...
						if (true == childNodes.get(j).getAttributes().get(0).getEntity().equals(
								rootChildNodes.get(i).getAttributes().get(0).getEntity()))
						{
							pos = j;
							break;
						}
