...
public class CategoryCache {
	...
	private void init(Connection conn) {
		...
		categoryVsClasseSet = new HashMap<Long, Set<EntityInterface>>(
				categories.size());
		categoryVsAttributeSet = new HashMap<Long, Set<AttributeInterface>>(
				categories.size());
	...
	public Set<EntityInterface> getAllSourceClasses(Category category) {
		return new HashSet<EntityInterface>(categoryVsClasseSet.get(category
				.getId())); // avoid modification from outside
	}
	...
	public Set<AttributeInterface> getAllSourceAttributes(Category category) {
		return new HashSet<AttributeInterface>(categoryVsAttributeSet
				.get(category.getId())); // avoid modification from outside
	}

	private void addCategoryToCache(Category category) {
		CategoryOperations categoryOperations = new CategoryOperations();
		categoryVsClasseSet.put(category.getId(), categoryOperations
	...
	public Category getCategoryByEntityId(Long id) {
		...
		if (category == null) {
			throw new RuntimeException(
					"Category with given entity id not found.");
		}
		return category;
	}
