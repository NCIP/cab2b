...
public class CategoryOperations extends DefaultBizLogic {
	...
	public Set<EntityInterface> getAllSourceClasses(Category category) {
		...
		Set<EntityInterface> entities = new HashSet<EntityInterface>(idSet
				.size());
		for (Long id : idSet) {
	 ...
	 *            param.
	 */
	private void getDeEntityIdsOfChildren(
			CategorialClass parentCategorialClass, Set<Long> idSet) {

		for (CategorialClass categorialClass : parentCategorialClass
				.getChildren()) {
	...
	private void processCategorialClass(CategorialClass categorialClass,
		...
		for (CategorialAttribute attribute : categorialClass
			...
			long categoryEntityAttributeId = attribute
					.getDeCategoryAttributeId();
	...
	public List<EntityInterface> getAllRootCategories() {
		...
		try {
			allCategories = retrieve(Category.class.getName(), null,
					new String[] { "parentCategory" },
					new String[] { "is null" }, new Object[0], null);
		} catch (DAOException e) {
			throw new RuntimeException("Error in fetching category", e,
					ErrorCodeConstants.CATEGORY_RETRIEVE_ERROR);
		}
		List<EntityInterface> categoryEntities = new ArrayList<EntityInterface>(
				allCategories.size());
		EntityCache cache = EntityCache.getInstance();
	...
	public Set<AttributeInterface> getAllSourceAttributes(Category category) {
		Set<AttributeInterface> attributeSet = getAllChilrenCategorialClasses(category
				.getRootClass());
		return attributeSet;
	 ...
	 */
	private Set<AttributeInterface> getAllChilrenCategorialClasses(
		...
		for (CategorialAttribute catAttr : catClass
				.getCategorialAttributeCollection()) {
