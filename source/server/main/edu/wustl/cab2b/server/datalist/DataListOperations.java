...
public class DataListOperations extends DefaultBizLogic
{
	...
	public List<DataListMetadata> retrieveAllDataListMetadata() throws DAOException, ClassNotFoundException
	{
	 ...
	 */
    public Long save(DataList dataListToSave) throws DynamicExtensionsApplicationException, DynamicExtensionsSystemException
    {
		...
		for (IDataRow firstLevelTypeDataRow : rootDataRow.getChildren())
		{
			...
			if(firstLevelTypeDataRow.isData() == false)
			{
				...
				{
					EntityInterface newCurrentEntity = mapOfNewEntities.get(firstLevelDataRow.getEntityInterface().getName()+"_"+2);
					if(newCurrentEntity == null)
					{
						newCurrentEntity = getNewEntity(firstLevelDataRow.getEntityInterface());
						mapOfNewEntities.put(firstLevelDataRow.getEntityInterface().getName()+"_"+2, newCurrentEntity);
					}
					
					newAssociation = mapOfNewAssociations.get(dataListEntity.getName()+"_"+firstLevelDataRow.getEntityInterface().getName());
					
					if(newAssociation == null)
					{
						newAssociation = createNewOneToManyAsso(dataListEntity, newCurrentEntity);
						mapOfNewAssociations.put(dataListEntity.getName()+"_"+firstLevelDataRow.getEntityInterface().getName(), newAssociation);
					...
					constructDataRowMap(firstLevelDataRow, mapForFirstLevelDataRow, newCurrentEntity, 3);
			...
			{
				EntityInterface newCurrentEntity = mapOfNewEntities.get(firstLevelTypeDataRow.getEntityInterface().getName()+"_"+2);
				if(newCurrentEntity == null)
				{
					newCurrentEntity = getNewEntity(firstLevelTypeDataRow.getEntityInterface());
					mapOfNewEntities.put(firstLevelTypeDataRow.getEntityInterface().getName()+"_"+2, newCurrentEntity);
				}
				
				newAssociation = mapOfNewAssociations.get(dataListEntity.getName()+"_"+firstLevelTypeDataRow.getEntityInterface().getName());
				if(newAssociation == null)
				{
					newAssociation = createNewOneToManyAsso(dataListEntity, newCurrentEntity);
					mapOfNewAssociations.put(dataListEntity.getName()+"_"+firstLevelTypeDataRow.getEntityInterface().getName(), newAssociation);
					dataListAttributesMap.put(newAssociation, new ArrayList());
					dataListEntity.addAssociation(newAssociation);
				}
				List<Map> listOfSubMaps = (List<Map>)dataListAttributesMap.get(newAssociation);
				...
				Map<AbstractAttributeInterface, Object> mapForFirstLevelDataRow = new HashMap<AbstractAttributeInterface, Object>();
				constructDataRowMap(firstLevelTypeDataRow, mapForFirstLevelDataRow, newCurrentEntity, 3);
		...
		Logger.out.info("DataList saved successfully !!!! (entityId, recordId) :: "+entityId+", "+recordId);
    ...
    private AssociationInterface createNewOneToManyAsso(EntityInterface srcEntity, EntityInterface tarEntity)
	{
		...
		returner.setTargetRole(getNewRole(AssociationType.CONTAINTMENT, "target_role_"+associationName, Cardinality.ZERO,
				Cardinality.MANY));
	 ...
	 * @throws DynamicExtensionsSystemException
	 */
	private void constructDataRowMap(IDataRow dataRowToSave, Map<AbstractAttributeInterface, Object> mapToConstruct, EntityInterface newEntity, int treeLevel)
			throws DynamicExtensionsApplicationException, DynamicExtensionsSystemException
	{
		...
		if (values != null)
		{
			...
			List<AttributeInterface> newAttributes = getOrderedAttributes(dataRowToSave.getAttributes(), newEntity.getAttributeCollection());
			...
			if (children != null && children.size() > 0)
			{
				...
				{					
					EntityInterface newChildEntity = mapOfNewEntities.get(dataRow.getEntityInterface().getName()+"_"+treeLevel); //getNewEntity(dataRow.getEntityInterface());
					if(newChildEntity == null)
					{
						newChildEntity = getNewEntity(dataRow.getEntityInterface());
						mapOfNewEntities.put(dataRow.getEntityInterface().getName()+"_"+treeLevel, newChildEntity);
					}
					...
					//Logger.out.info("listOfMapToAppend "+listOfMapToAppend);
				}
			}
		}
		else
		{
			List<IDataRow> children = dataRowToSave.getChildren();
			//Logger.out.info("in else children.size()  ####  " + children.size());
			if (children != null)
			{
				for (IDataRow dataRow : children)
				{
	 ...
	 * @return
	 */
	private List<AttributeInterface> getOrderedAttributes(List<AttributeInterface> orderedOldAttribs, Collection<AttributeInterface> unOrderedNewAttribs)
	{
