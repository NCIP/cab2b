...
public class ConstraintsBuilder {
	private static final Logger logger = edu.wustl.common.util.logger.Logger
			.getLogger(ConstraintsBuilder.class);
	 ...
	 */
	public ConstraintsBuilder(ICab2bQuery query,
	...
	private Set<Integer> getCategoryOutputExpressionIds() {
		Set<TreeNode<IExpression>> outputExpressions = getCategoryPreprocessorResult()
				.getExprsSourcedFromCategories().get(
						getQuery().getOutputEntity());
		Set<Integer> outputExpressionIds = new HashSet<Integer>(
	...
	private void constrainByParentExpressions() {
		...
		while (!currExprIds.isEmpty()) {
			...
			for (Integer currExprId : currExprIds) {
				...
				List<Integer> parentExprIds = exprIdList(getJoinGraph()
						.getParentList(currExpr));
				...
				boolean isOutputExpr = (isCategoryOutput && outputExprIds
						.contains(currExprId))
						|| currExpr.getQueryEntity().equals(getOutputEntity());
				if (!isOutputExpr) {
					nextExprIds.addAll(exprIdList(getJoinGraph()
							.getChildrenList(currExpr)));
				}
	...
	private void constrainChildByParents(int childExprId,
			List<Integer> parentExprIds) {
		IExpression childExpr = getExpression(childExprId);
		DcqlConstraint childConstraint = getResult()
				.getConstraintForExpression(childExpr);
		...
		for (Integer parentExprId : parentExprIds) {
			...
			DcqlConstraint parentConstraint = getResult()
					.getConstraintForExpression(parentExpr);
		...
		for (int i = 0; i < expr.numberOfOperands(); i++) {
			...
			if (operand instanceof IExpression) {
				...
				dcqlConstraintsList.add(createDcqlConstraintForChildExpression(
						expr.getExpressionId(), childExpr.getExpressionId()));
				// }
			} else if (operand instanceof IRule) {
				dcqlConstraintsList
						.add(createDcqlConstraintForRule((IRule) operand));
			} else {
	...
	private DcqlConstraint mergeConstraints(IExpression expr,
		...
		return new GroupBuilder(getConnectors(expr), dcqlConstraintsList)
				.buildGroup();
	}

	private List<IConnector<LogicalOperator>> getConnectors(
	...
	private DcqlConstraint createDcqlConstraintForChildExpression(
			int parentExpressionId, int childExpressionId) {
		...
		if (isExprRedundant(childExpressionId)) {
			...
			IAssociation association = getJoinGraph().getAssociation(
					getExpression(parentExpressionId), childExpr);
	...
	private static LocalAssociationConstraint createLocalAssociation(
		...
		AssociationInterface deAssociation = intraModelAssociation
	...
	private static ForeignAssociationConstraint createForeignAssociation(
		...
		foreignObject
				.setName(interModelAssociation.getTargetEntity().getName());
		association.setTargetServiceURL(rightServiceUrl);

		// 2. create join condition

		joinCondition.setLocalAttributeName(interModelAssociation
				.getSourceAttribute().getName());
		joinCondition.setForeignAttributeName(interModelAssociation
	...
	private DcqlConstraint createDcqlConstraintForRule(IRule rule) {
		...
		for (int i = 0; i < rule.size(); i++) {
			group.addConstraint((createConstraintForCondition(rule
					.getCondition(i))));
		}
	...
	private AttributeConstraint createAttribute(String attributeName,
			RelationalOperator operator, DataType dataType) {
		return createAttributeConstraint(attributeName, operator, null,
				dataType);
	}
	...
	public static AttributeConstraint createAttributeConstraint(
			String attributeName, RelationalOperator operator, String value,
			DataType dataType) {
	...
	private DcqlConstraint createBetweenCondition(String attributeName,
			String value1, String value2, DataType dataType) {
		Cab2bGroup group = new Cab2bGroup(LogicalOperator.And);
		boolean swapVals;
		switch (dataType) {
		case Integer:
		...
		group.addConstraint((createAttributeConstraint(attributeName,
				RelationalOperator.GreaterThanOrEquals, value1, dataType)));
		group.addConstraint((createAttributeConstraint(attributeName,
				RelationalOperator.LessThanOrEquals, value2, dataType)));
		return group.getDcqlConstraint();
	}
	...
	private DcqlConstraint createInCondition(String attributeName,
		...
		for (String value : values) {
			group.addConstraint((createAttributeConstraint(attributeName,
					RelationalOperator.Equals, value, dataType)));
		}
		return group.getDcqlConstraint();
	}
	...
	private DcqlConstraint createNotInCondition(String attributeName,
		...
		for (String value : values) {
			group.addConstraint((createAttributeConstraint(attributeName,
					RelationalOperator.NotEquals, value, dataType)));
		}
		return group.getDcqlConstraint();
	}
	...
	private DcqlConstraint createConstraintForCondition(ICondition condition) {
		...
		DataType dataType = Utility.getDataType(condition.getAttribute()
		...
		if (operator.numberOfValuesRequired() == 1) {
			return createAttributeConstraint(attributeName, operator, condition
					.getValue(), dataType);
		}

		if (condition.getRelationalOperator() == RelationalOperator.Between) {
			return createBetweenCondition(attributeName, condition.getValues()
					.get(0), condition.getValues().get(1), dataType);
		}
